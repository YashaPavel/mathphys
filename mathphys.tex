...
$\supp\phi={x: |x|<1}$, его замыкание даёт [-1; 1], что является компактным множеством. $\dfrac{1}{x^2-1}$ - гладкая на (-1, 1), поэтому экспонента тоже гладкая, значит $\in\Cinf(\R)$.
...
\end{mylemma}
\begin{myproof}
    С произведением всё очевидно, перемножение так же даёт бесконечно гладкую функицю и так же производные убывают быстрее любой степени.
...
\lbpn{furie}{$$f_k\xrightarrow{\SSR}f \quad \Rightarrow \quad \widehat{f_k}\xrightarrow{\SSR}\widehat{f}$$}
...
\end{myproof}
Если $f\in L_1(\R)$ задаёт регулярную обобщённую функцию $\tilde{f}\in\SSR$. Мы знаем, что $\widehat{f}$ - ограниченная (\ref{seventh}), а ограниченная функция - так же регулярная обобщённая функция, то есть $\tilde{\widehat{f}}$, нас интересует $\tilde{\widehat{f}}\overset{?}{=}\widehat{\tilde{f}}$
\begin{mylemma}
...
\begin{myexe}
    ...
    $f\ast\psi\in\SSR$\\
По определению сходимости в $\SR$ (Определение 1.9 \eqref{1.9}),
...
Поскольку $\psi_k \xrightarrow{\SR} 0$, по определению 1.9\eqref{1.9}
...
По определению 1.9 \eqref{1.9} это означает, что
\[
...
\begin{mydef}
    ...
    Так же мы получаем, что $n_{0}=1\text{(нет вторых производных)},$ $n_+=0 \text{(с минусом оператор)}$. 
\end{mydef}
\begin{mydef}
...
\begin{mytheorem}
    \lbpn{true}{Пусть \(\phi\in\C^2(\R), \,\,\psi\in\C(\R) \quad \Rightarrow \quad \exists! \,\,\, U\in\C^2([0, +\infty)\times R)\) - решения задачи \ref{volna} и формула Даламбера является выражением для него}
\end{mytheorem}
Свойства решения: \\
1) Пусть $\psi\equiv0$: 
\begin{figure}[H]
        ...
        \label{fig:enter-label}
\end{figure} 
Здесь на картинке пики как бы смещаются на $ct$ влево и вправо и еще в 2 рааз уменьшаются.\\
$\supp \phi\subset[a, b]$
\begin{figure}[H]
        \centering
...
только в $\mathcal{S}'(\R^3)$. Мы верим, что решение будет из $S'(\R^3)$. Решение уже последнего уравнения имеет знакомый нам вид:
\[
...
\]
Тогда понятно, что \(\hat{g}(t, \cdot)\in\Cinf(\R^3)\), на бесконечности она убывает, \(\hat{g}(t, \cdot)\in\mathcal{S}'(\R^3)\). Но она не интегрируема из-за того что степень знаменателя должна быть больше размерности, но у нас она единица, поэтому \(\hat{g(t,\cdot)}\notin L_1(\R^3)\). Будем это исправлять. \\
...
\begin{mytheorem}
    Предположим, что у однородной задачи \eqref{78} ($f\equiv0$), существует только тривиальное решение ($y\equiv0$), тогда:
...
\begin{mydef}
    Функцию Грина для задачи \eqref{78}, можно записать еще и следующим образом:
    \[
\begin{cases}
...
\begin{mytheorem}
    Предположим, что у однородной задачи \eqref{78}, существует нетривиальное решение $y_0,$ тогда у неоднородного задачи либо нет решений, либо их бесконечно много
\end{mytheorem}
...
\begin{myremark}
    ...
    \begin{enumerate}
        ...
        \\
        Если у нас есть базис из собственных векторов, $A\vec{u_k}=\lambda \vec{u_k}$, то мы все можем разлodить по этому базису:
...
\begin{myproof}
    ...
    Чтобы понять, какая, именно это константа, рассомотрим значение выше введеного соотношения в точке $a$. Так как $u(a)=v(a)=0$, то:
...
\begin{myproof}
    ...
    \[\begin{cases}
        y(x_0)=0\\
        y'(x_0)=0
    \end{cases} \Rightarrow \text{по теореме о существование единственного решения} \Rightarrow y \equiv 0
    \]
\end{myproof}
...
\begin{myremark}
    Пусть у функции $\psi$ есть нули на $[a,b]$, обозначим их за $z_j(\lambda)$ и расположим в порядке возрастания, тогда:
...
Мы имееем бесконечный набор положительных собстенных чисел $\lambda_n \xrightarrow[n\to \infty] \ +\infty$ и соответсвующих им собственных функций $y_n$, набор которых полон. Другими словами:$\forall f\in\L_2([a,b]),$ в разложении ряд сходиться, то есть $$\int\limits_{a}^{b}\Bigg|f(x)-\sum_{n=1}^{k}f_n(t)y_n(x)\Bigg|^2\dd x \xrightarrow{k \to \infty} 0 \quad \text{ в }\L_2([a,b])$$ или же:
...
Таким образом, каждую функцию мы можем разлoжить в ряд фурье по собсвтенным функциям $y_n$:
...
энергии в промежутке времени от $t_1$ до $t_2$. Энергия представляет в виде $\int c\rho u(t,x)\dd x$, тогда ее изменение приминает вид:
...
И отсюда прекрасно видно, что для леммы все выполненно, тогда теорема верна.
...
\begin{enumerate}
    ...
    С минимум доказательство аналогично, только везде надо будет поменять знак неравенств.
...
\[\dfrac{\p u_2}{\p t} - \Delta_x u_2 = \int\limits_{0}^{t}\int\limits_{\R^n}g(\tau,y)\left(\dfrac{\p f}{\p t}(t-\tau, x-y) - \Delta_x f(t-\tau, x-y)\right)\dd y\dd \tau+\]\[+\int\limits_{\R^n}g(t,y)f(0, x-y)\dd y :=I_1+I_2\]
    Теперь давайте рассмотрим $I_1$, взяв $\epsilon>0$ и разобьем по промежутку:
    \[
    I_1 = \int\limits_0^\epsilon\int\limits_{\R^n}\cdots+\int\limits_\epsilon^{t}\int\limits_{\R^n}\cdots = I_{0,\epsilon} + I_{\epsilon,t}
    \]
 Ограничим подыинтегральное выражение следующим образом:
    \[
...
\begin{myexe}
    \begin{enumerate}
        \item Если есть функция $v\in C^{1,2}(Q)\cap C(\overline{Q})$, максимум которой случился в некоторой точке $(t_1,x_1)$:
\[
...
\]
Сейчас посмотрим на сходимость на произовольной области $\D'(\Omega)$. Пусть $\phi\in C^{\infty}_0(a,b)$,тогда ряд $\phi(x) = \SUMN\ \phi_ny_n(x)$ сходится в $C^{\infty}_0(a,b)$, тогда:
...
и рассмотрим произвольные точки $x,y$ внутри шара $B_R(x_0)$. Для таких точек очевидно следующее соотношение(неравенство треугольника):
\[|x-y|\leq|x-x_0|+|y-x_0|<2R\]
Следующим шагом посмотрим на шар $B_R(x)$ и точку $z$, внутри него. Запишем внимательно аналогичное неравенство треугольника:
...
Понимая, что в такой задаче внутреняя нормаль, можем записать ядро Пуассона:
\[
