...
\end{myremark}
\begin{myexample}
    1)$f_k(x)=\left[\begin{gathered}
        k, \quad 0<x<\frac{1}{k} &
        0, \quad \text{иначе}
    \end{gathered} \xrightarrow{\DDR}\delta$
    \begin{figure}[H]
        \centering
...
\end{mylemma}
\begin{myproof}
    Если $x\in[C, D]$, т.е. $x\not\in[A,B]$ то $\exists O(x):f|_{O(x)}=0 \, \forall x\in[C, D], \, [C, D]\underset{X}{\subset}$-покрытие $[C,D]$ этими окрестностями. $[C, D]$-компакт $\Rightarrow [C, D]\subset\cup_{j=1}^{N}\theta(x_j)$ - конечное подпокрытие.
...
\end{myexe}
\begin{mytheorem}
    $$f\in\DDR, \supp f = \{0\} \Rightarrow\exists k\in \N_0, c_0, c_1, \cdots c_k \in C \quad f=\sum_{j=0}^{k}c_j\delta^{(j)}$$
\end{mytheorem}
\begin{myexe}
...
\[V(t,\xi)=\dfrac{1}{(2\pi)^{n/2}}\int_{\r^n}e^{-i\xi x }u(t,x)\dd x=F(u)\] Тогда задача принимает следующий вид:
