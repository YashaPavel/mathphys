...
\begin{enumerate}
    ...
    \item Гельфанд И.\,М., Шилов Г.\,Е. 
    \textit{Пространства основных и обобщённых функций}.

    \item Смирнов В.\,И. 
    \textit{Курс высшей математики}. Том 3.

    \item Тихонов А.\,Н., Самарский А.\,А. 
    \textit{Уравнения математической физики}.

    \item Шубин М.\,А. 
    \textit{Лекции об уравнениях математической физики}.
\end{enumerate}
...
\section{Определения}
\subsection{Класс основных обобщённых функций \texorpdfstring{$\DR$}{D(R)}}
...
Носитель функции равен $\supp\varphi=[-1,1]$, поэтому он компактен. На интервале $(-1,1) $ функция $\dfrac{1}{x^2-1}$ гладкая, следовательно, экспонента также гладкая. В точках $x=\pm1$ функция вместе со всеми производными стремится к нулю, поэтому $\varphi\in C^\infty(\R)$ и, следовательно, $\varphi\in\DR$.

\end{myexe}
Таким образом, $\DR$ является линейным пространством.

\begin{mydef}\label{zeroo}
...
\begin{myexe}
    Показать, что $\DR$ не метризуемо, то есть не существует метрики $\rho$, для которой: $\rho(\varphi_k,\varphi) \rightarrow0 \Leftrightarrow \varphi_k \xrightarrow{\DR} \varphi$
\begin{comment}
\begin{figure}[H]
        \centering
...
\subsection{Класс обобщённых функций \texorpdfstring{$\DDR$}{ }}

\begin{mydef}
    \[f\in\DDR \Leftrightarrow f -- \text{линейный непрерывный функционал на }\, \DR\]
    \[f:\DR \rightarrow \mathbb{C}\]
\end{mydef}
Другими словами, каждому $\varphi\in\DR$ функционал $f$ ставит в соответствие число $(f,\varphi)$:
...
\textbf{2) } Непрерывность: \\
Если $\varphi_k \xrightarrow{\DR} \varphi$, то $(f,\varphi_k)\rightarrow(f,\varphi)$


\begin{myremark}
    1) В англоязычной литературе обобщённые функции обычно называют distributions (распределениями).\\
    2) Непрерывность линейного функционала достаточно проверять в нуле.\\
...
\subsection{Регулярные и сингулярные обобщённые функции}
...
\[\varphi_k\xrightarrow{\DR}0\Rightarrow|(\Tilde{f},\varphi)|\leq \int\limits_{-R}^{R}f(x)|\varphi_k(x)|\dd x\leq \int\limits_{-R}^{R}|f(x)|\max_{x \in [-R,R]}|\varphi_k|\dd x\rightarrow0\]
Видно, что функция непрерывна в нуле $\Rightarrow$ непрерывна везде $\Rightarrow \Tilde{f}\in \DDR$ 

\begin{mydef}
    Обобщённая функция $\Tilde{f}$, порождённая локально интегрируемой функцией $f$, называется \textit{регулярной}; все остальные обобщённые функции называются \textit{сингулярными}.
    \[f:\R\rightarrow\mathbb{C}\]
    \[\Tilde{f}:\DR\rightarrow\mathbb{C}\]
\end{mydef}


\begin{mytheorem}\label{thm:regular-distribution-zero}
    Пусть $f\in L_{1,loc}$ и $\int\limits_{\R}f(x)\varphi(x)\dd x=0 \,\text{для всех } \varphi\in\DR\Rightarrow f =0$
\end{mytheorem}
Рассмотрим примеры сингулярных обобщённых функций.
\begin{myexample}
    

\begin{enumerate}
\item $\delta$ — сингулярная обобщённая функция
\[
(\delta, \varphi) = \varphi(0)
\]
...
Возьмём $\varphi \in \mathcal{D}(\R)$ и 
\[
...
\]
\[
\Rightarrow \textbf{по теореме \ref{thm:regular-distribution-zero}}
...
\[
\Rightarrow \textbf{получаем противоречие}.
\]
...
\item 
$\PP$ -- сингулярная обобщённая функция.
...
\end{myexample}
\begin{myexe}
    Показать, что функция $\px{x}$ -- сингулярная обобщённая функция
Пусть $\varphi \in \DR$ и
\[
\psi(x) = x \varphi(x) \in \DR
\]

По формуле Сохоцкого $\dfrac{1}{x+i0}$ имеем
\[
\left\langle \frac{1}{x+i0}, \psi \right\rangle
= \operatorname{v.p.} \int\limits_{\R} \frac{\psi(x)}{x}\,\dd x
= \operatorname{v.p.} \int\limits_{\R} \varphi(x)\,\dd x 
\]
...
\[
\operatorname{v.p.} \int\limits_{\R} \varphi(x)\,\dd x
= \int\limits_{\R} x f(x)\,\varphi(x)\,\dd x 
\]
...
\[
\int\limits_{-1}^{1} \left|\frac{1}{x}\right|\,\dd x = +\infty 
\]
...
\section{Доказательство теоремы \ref{thm:regular-distribution-zero}}
...
\int\limits_{\R} g(x)\varphi(x)\,\dd x 
=
\int\limits_{a-\delta}^{a+\delta} g(x)\varphi(x)\,\dd x 
\]
...
\int\limits_{a-\delta}^{a+\delta} g(x)\varphi(x)\,\dd x 
\;\ge\;
\int\limits_{a-\delta}^{a+\delta} \varepsilon\,\varphi(x)\,\dd x 
=
\varepsilon \int\limits_{a-\delta}^{a+\delta} \varphi(x)\,\dd x 
\]
...
\begin{myremark}
    $$\nexists \, c\circ f$$ 
    Такая композиция в общем случае не определена (например, выражение $(\delta(x))^2$ не имеет смысла в стандартной теории распределений).
\end{myremark}
...
\\
1) Обозначим через $T$ распределение, действующеё по формуле
\[
...
\[
-\int_{0}^{\infty} \ln(x)\,\varphi'(x)\,\dd x 
= \int_{0}^{\infty} \frac{1}{x}\,\varphi(x)\,\dd x 
\qquad
-\int_{-\infty}^{0} \ln(-x)\,\varphi'(x)\,\dd x 
= \int_{-\infty}^{0} \frac{1}{x}\,\varphi(x)\,\dd x 
\]
Следовательно,
\[
\langle T',\varphi\rangle
= \int_{\R} \frac{1}{x}\,\varphi(x)\,\dd x 
...
\[
\varphi \mapsto c\int_{\R} \varphi(x)\,\dd x 
\]
...
= -\int_{\mathbb{R}} f(x)\varphi'(x)\,\dd x 
\]
Разобьём интеграл по точке $0$:
\[
-\int_{\mathbb{R}} f(x)\varphi'(x)\,\dd x 
= -\int_{-\infty}^{0} f(x)\varphi'(x)\,\dd x 
  -\int_{0}^{\infty} f(x)\varphi'(x)\,\dd x 
\]
Интегрируя по частям на каждом интервале и используя компактность
носителя $\varphi$, получаем
\[
...
= -\bigl[f(x)\varphi(x)\bigr]_{-\infty}^{0}
   + \int_{-\infty}^{0} f'(x)\varphi(x)\, \dd x 
= -f(-0)\varphi(0) + \int_{-\infty}^{0} f'(x)\varphi(x)\,\dd x 
\]
\[
-\int_{0}^{\infty} f(x)\varphi'(x)\,\dd x 
= -\bigl[f(x)\varphi(x)\bigr]_{0}^{\infty}
   + \int_{0}^{\infty} f'(x)\varphi(x)\,\dd x 
= f(+0)\varphi(0) + \int_{0}^{\infty} f'(x)\varphi(x)\,\dd x 
\]
Складывая, имеем
\[
\langle f'_{\text{обобщ.}},\varphi\rangle
...
\end{myexe}
\begin{mytheorem}
    Пусть $f_k$ кусочно непрерывны и $f_k\in L_1(\R)$, причём \\
    1) $f_k(x)\geqslant0$ почти всюду. \\
    2) $\forall a>0 \, \int\limits_{-a}^{a}f_k(x)\dd x \xrightarrow[k\to\infty]{} 1$ \\
    3) $\int\limits_{\R}f_k(x)\dd x=1$ \\
Тогда $f_k\xrightarrow{\DDR}\delta(x)$.
\end{mytheorem}
\begin{myproof}
    $$\int\limits_\R f_k(x)\phi(x)\dd x \longrightarrow \phi(0) \, ?$$
    Пусть $\phi\in\DR$. Поскольку $\phi$ непрерывна, для любого $\varepsilon>0$ существует $\delta>0$ такое, что $\exists \delta>0: \, |\phi(x)-\phi(0)|<\frac{\varepsilon}{2}$
...
\end{myproof}
\begin{myremark}
    1) Если $\phi$ непрерывна и ограничена на $\R \Rightarrow \int\limits_Rf_k\phi \dd x \rightarrow \phi(0)$ 
\end{myremark}
\begin{myexe}
    Показать, что третье условие теоремы избыточно. \\ 
...
\end{myexe}
\begin{myremark}
    Последовательность $\{f_k\}$ называется $\delta$-образной.
\end{myremark}
\begin{myexample}\label{fact}
    1)\[
f_k(x)=
\begin{cases}
k, & 0<x<\dfrac{1}{k},\\
...
\begin{myproof}
    ...
    $$(f, \psi)=(f, \sum_{j=1}^{N}\xi_j\psi)=\sum_{j=1}^{N}(f, \xi_j\psi)=0, \, \text{так как } \supp(\xi_j\psi)\subset O(x_j)(\text{так как }\xi_j\psi\in\DR)$$
\end{myproof}
\begin{mytheorem}
    Пусть $f\in\DDR, \supp f$ - компактный, пусть $m\in\N_0, \exists g\in\C(\R): \, f=g^{(m)}$(производная в смысле обобщённой функции)
\end{mytheorem}
\begin{myproof}
    без доказательства
\end{myproof}
...
\end{mytheorem}
\begin{myexe}
    Доказать эту теорему, используя предыдущую теорему и упражнение из раздела 1.3.4.
...
\end{myexe}
\section{Прямое произведение обобщённых функций}
...
$f, g\in\DDR \quad \int\limits_{\R^2}f(x)g(y)\phi(x, y)\dd x\dd y$ - это то, чего мы пытаемся добиться прямым произведением.
\begin{mylemma}\label{second}
    Пусть $\phi\in\D(\R^2) $, пусть $x\in\R$ - фиксированный.
...
\end{myproof}
\begin{mylemma}
    Пусть $g\in\DDR, \, \varphi \in\D(\R^2) $. 
...
\begin{myproof}
    ...
    Достаточно доказать, что $\psi_k\xrightarrow[k\to+\infty]{}0$ равномерно (в силу леммы 1.2(потому что производные строятся точно также)). Пойдём от противного (сходится неравномерно): $\exists\varepsilon>0, \exists k_j(\to+\infty),x_j:|\psi_{k_j}(x_j)|\geq\varepsilon \Leftrightarrow |(g,\phi_{k_j}(x_j, \cdot))|\geq\varepsilon$
...
Пусть $\phi, \psi \in \DR$. Для обычных функций свёртка имеет вид: 
...
Последнее равенство обусловлено тем, что обобщённая функция $f$ действует по переменной $\cdot$.
\end{myproof}
...
\begin{myremark}
    ...
    2) Свёртку двух произвольных обобщённых функций определить, вообще говоря, нельзя: возникающая тестовая функция может не иметь компактного носителя. $\phi=\psi=1$. Тогда 
    ...
    Но здесь есть проблема: $\phi(x,y):=\phi(x+y)$, здесь носитель не компактный, это можно увидеть на графике:
 \begin{figure}[H]
        \centering
...
\end{myexe}
Определим свёртку обобщённых функций правильно.
\begin{mydef}
    Пусть $f, g\in\DDR \quad \supp f \subset(a, b)$. 
    $$(f\ast g, \phi)=(f(x)g(y),\underbrace{\phi(x+y)\eta(x)}_{\in\D(\R^2)})$$
\end{mydef}
Геометрически условие на носитель выделяет на рисунке параллелограмм.
\begin{myexe}
...
\begin{myremark}
    ...
    3) $\delta\ast f=f$ \\
    4) Можно также дифференцировать: $(f\ast g)'=f'\ast g=f\ast g'$. \\
    $$(\{\text{обобщённые функции с компактным }\supp\}, \ast) - \text{ коммутативная алгебра с единицей}$$
    Контрпример - $\theta, \delta', 1$ и это не будет алгеброй: \\
...
На всём пространстве $\DDR$ преобразование Фурье определить нельзя, поэтому класс обобщённых функций необходимо сузить, а класс основных функций — расширить.
\begin{mydef}
...
\end{mydef}
\begin{myremark}\label{forth}
    $$\phi\in\SR \quad \Rightarrow \quad x\phi\in\SR$$
    Домножение возможно, так как убывает быстрее любой степени и функция также остаётся гладкой.
\end{myremark}
\begin{myexe}
    1) $\DR\subset\SR$ \\
...
\end{myexe}
\begin{mydef} \label{def:schwartz-convergence}
...
\end{mylemma}
\begin{myproof}
    Для произведения всё следует из формулы Лейбница: произведение функций остаётся бесконечно гладким, а все его производные убывают быстрее любой степени.
...
Последнее равенство получено с помощью формулы бинома Ньютона. По определению функции из пространства Шварца: 
...
После интегрирования по частям граничный член равен нулю, поскольку $\phi$ быстро убывает.
\begin{equation}
\label{third}
\widehat{\phi'}=i\xi\hat{\phi}(\xi) \quad (\phi^{(l)})\hat{}=(i\xi)^l\widehat{\phi}(\xi)
\end{equation}
\begin{myremark}\label{seventh}
    Преобразование Фурье можно ввести не только на классе Шварца: $\phi\in\L_1(\R)\Rightarrow \quad \widehat{\phi}$ - ограниченная функция, так как 
...
$$\xi^m\widehat{\phi}(\xi)=(-i)^m(\phi^{(m)})\hat{} \text{  - ограничена по предыдущему замечанию, так как} \quad \int\limits_\R|\phi^{(m)}|\dd x<\infty$$
...
\end{mydef}
\begin{mytheorem}\label{fifth}
    Пусть $\phi\in\SR$, тогда: 
    $$\F^{-1}\F\phi=\F\F^{-1}\phi=\phi$$
\end{mytheorem}
\begin{myproof}
...
\begin{myexe}\label{sixth}
    $$\F: \,\, \SR\rightarrow\SR\text{ - непрерывный}$$
Пусть
\[
...
\end{mylemma}
\begin{myproof}
    $(\phi\ast\psi)\hat{}(\xi)=\dfrac{1}{\sqrt{2\pi}}\int\limits_\R\int\limits_\R e^{-i\xi x}\phi(x-y)\psi(y)\dd y\dd x\underset{x=y+z}{=}\dfrac{1}{\sqrt{2\pi}}\int\limits_\R\int\limits_\R e^{-i\xi y-i\xi z}\phi(z)\psi(y)\dd y\dd z=\dfrac{1}{\sqrt{2\pi}}\int\limits_\R e^{-i\xi y}\phi(y)\dd y\int\limits_\R e^{-i\xi z}\phi(z)\dd z=\sqrt{2\pi}\widehat{\phi}(\xi)\widehat{\psi}(\xi)$
\end{myproof}
\begin{mylemma}
    Следствие из предыдущей леммы: 
    $$(\phi\psi)\hat{}=\frac{1}{\sqrt{2\pi}}\widehat{\phi}\ast\widehat{\psi}$$
\end{mylemma}
\begin{myproof}
...
\subsection{Пространство медленно растущих обобщённых функций \texorpdfstring{$\SSR$}{ }}
\begin{mydef}
    $$f\in\SSR \quad \Leftrightarrow \quad f \text{ - линейный непрерывный функционал на } \SR$$
\end{mydef}
Важно, что непрерывный относительно той сходимости, которую мы завели на $\SR$. Здесь также можно определить регулярные функции, но здесь локальной интегрируемости не хватит из-за бесконечности. \\
Пусть $f\in\SR: \,\,\, \exists N: \quad \int\limits_\R\dfrac{|f(x)|}{(1+|x|)^N}\dd x<\infty$. Отсюда автоматически следует, что $f\in\L_{1, loc}$. Тогда заведём обобщённую функцию: 
$$(\tilde{f}, \phi)=\int\limits_\R f(x)\phi(x)\dd x=\int\limits_\R \underbrace{\dfrac{f(x)}{(1+|x|)^N}}_{\text{интегрируема}}\underbrace{\phi(x)(1+|x|)^N}_{ограничена}\dd x$$
Получаем, что $\tilde{f}$ регулярная обобщённая функция.
\begin{myexample}
    1) $\SSR\subset\DDR$ \\
...
\begin{mydef}
    Пусть $f\in\SSR$. Тогда преобразование Фурье определяется формулой: 
    $$(\widehat{f}, \phi)=(f, \widehat{\phi})$$
\end{mydef}
Пространства $\DR$ нам не хватило, потому что преобразование Фурье размывает носитель у этих функций. В $\DR$ если у функции $\phi$ и $\widehat{\phi}$ компактный носитель, то это тождественный 0. \\
...
$F: \,\, \SSR\rightarrow\SSR$ - линейный непрерывный оператор:
\begin{equation}
\label{furie}
f_k\xrightarrow{\SSR}f \quad \Rightarrow \quad \widehat{f_k}\xrightarrow{\SSR}\widehat{f}
\end{equation}
...
\end{myproof}
Если $f\in L_1(\R)$ задаёт регулярную обобщённую функцию $\tilde{f}\in\SSR$. Мы знаем, что $\widehat{f}$ - ограниченная (\ref{seventh}), а ограниченная функция - также регулярная обобщённая функция, то есть $\tilde{\widehat{f}}$, нас интересует $\tilde{\widehat{f}}\overset{?}{=}\widehat{\tilde{f}}$
\begin{mylemma}
...
Рассмотрим сначала гауссовский интеграл при $\operatorname{Re} a>0$:
\[
...
\begin{myexe}
    ...
    $f\ast\psi\in\SSR$\\
По определению сходимости в $\SR$ (Определение 1.9 \ref{def:schwartz-convergence}),
...
Поскольку $\psi_k \xrightarrow{\SR} 0$, по определению 1.9\ref{def:schwartz-convergence}
...
По определению 1.9 \ref{def:schwartz-convergence} это означает, что
\[
...
\]
где $(f,\psi)$ обозначает действие распределения $f$ на пробную функцию
$\psi \in \SR$.
...
\begin{mytheorem}
    ...
    $$\Rightarrow \quad (f\ast\phi)\hat{}=\sqrt{2\pi}\widehat{f}\widehat{\phi}$$
\end{mytheorem}
\begin{myproof}
    Пусть $\psi\in\SR$ 
...
\begin{equation}
\label{eight}
\sum\limits_{i,j=1}^{n}a_{ij}(x)\dfrac{\partial^2U(x)}{\partial x_i \partial x_j}+\sum\limits_{i=1}^{n}b_i(x)\dfrac{\partial U(x)}{\partial x_i}+C(x)U(x)=f(x), \quad x\in\Omega\subset\R^n
\end{equation}
...
По закону инерции Сильвестра при конгруэнтном преобразовании симметричной матрицы невырожденной матрицей число положительных, отрицательных и нулевых собственных значений сохраняется (всего $n=n_+ +n_- +n_0$ собственных чисел по размерности пространства)(\href{https://en.wikipedia.org/wiki/Matrix_congruence}{Конгруэнтность}).\\Тогда мы сведём уравнение, c помощью диагонализации матрицы к:
...
Положительная определённость можно записать как: $\langle a\xi, \xi \rangle\geq\nu_0 |\xi|^2 \quad \forall \xi\in\R^n, \,\,\, \nu_0>0$ \\
Или же в терминах собственных чисел: $n_+=n, \,\,\, n_-=n_0=0$ \\
Также к эллиптическим мы будем относить и отрицательно определённую матрицу, т. е. матрицу у которой собственные числа: $n_-=n, \,\,\, n_+=n_0=0$
\end{mydef}
\begin{mydef}
    ...
    Также мы получаем, что $n_{0}=1\text{(нет вторых производных)},$ $n_+=0 \text{(с минусом оператор)}$. 
\end{mydef}
\begin{mydef}
...
\[\begin{cases}
    ...
    \end{cases}\]
Дополнительно потребуем корректности постановки задачи по Адамару: решение должно существовать, быть единственным и устойчиво зависеть от исходных данных.
\begin{myexample}
Пример Адамара: 
$$\dfrac{\partial^2 U}{\partial x^2}+\dfrac{\partial^2 U}{\partial y^2}=0, \quad (x,y)\in\R^2, y>0$$
$$U\big|_{y=0}=\phi_m, \,\,\,\, \dfrac{\partial U}{\partial y}\bigg|_{y=0}=0$$
...
Здесь нарушается непрерывная зависимость решения от исходных данных: краевые условия стремятся к нулю, тогда как решение становится неограниченным.
\end{myexample}
...
\begin{cases}
 ...
 \end{cases}
\end{equation}
Именно такую начальную задачу далее будем решать.
...
U(t,x)=f(x+ct)+g(x-ct)
\end{equation}
Это представление сохраняется в рассматриваемых классах функций. \\
...
\begin{mytheorem}\label{true}
    Пусть $\phi\in C^2(\R)$ и $\psi\in C(\R)$. Тогда существует единственное решение $U\in C^2([0,+\infty)\times\R)$ задачи~\ref{volna}, причём оно задаётся формулой Даламбера.
\end{mytheorem}
Свойства решения: \\
1) Пусть $\psi\equiv0$: 
\begin{figure}[H]
        ...
        \caption{}
\end{figure} 
На рисунке видно, что два профиля смещаются на $ct$ влево и вправо, а их амплитуды уменьшаются в два раза.\\
$\supp \phi\subset[a, b]$
\begin{figure}[H]
        \centering
        ...
        \caption{}
\end{figure}
На рисунке показан $\supp U$. Сечение при фиксированном $x$ даёт временной профиль волны; границы области распространения определяются характеристиками, $\tan\alpha=\dfrac{1}{c}$\\
...
\begin{mydef}\label{def:green-wave-1d}
    $g$ - функция Грина для уравнения струны $\quad\Leftrightarrow$ 
    \[\Leftrightarrow \quad g\in\Cinf((0,\infty), \DDR) \text{ и }\begin{cases}
...
\[g(t,x)=\dfrac{1}{2c}\int\limits_{x-ct}^{x+ct}\delta(y)\dd y\]
Формально такая запись требует аккуратной интерпретации в смысле распределений. Для дальнейших вычислений будем использовать эквивалентное представление:
\[g(t,x)=
\left[
\begin{array}{l}
...
Отсюда видно, что волновое уравнение \ref{def:green-wave-1d} выполняется. \\
...
\begin{mydef}
    ...
    \end{cases}\]
\end{mydef}
Остаётся проверить, что для любой $\phi\in\D(\R^2)$ выполняется:
...
\begin{myremark}
    В данном контексте термины «фундаментальное решение» и «функция Грина» часто используются как близкие по смыслу; далее будем придерживаться принятой в конспекте терминологии.
\end{myremark}
...
Здесь использовано свойство свёртки: производную можно переносить на любой из её множителей. \\
\begin{mytheorem}
...
\begin{myproof}
   ...
   Здесь мы использовали, что производная по верхнему пределу первого интеграла даст 0 из-за того, что при подстановке $\tau=t$ во втором интеграле обнуляется скобка $(t-\tau)$, поэтому останется только производная от переменных пределов второго интеграла. Отсюда видно, что \(\dfrac{\partial U_3}{\partial t}\bigg|_{t=0}=0\) \\
   ...
   \end{cases}\]
   Тогда получим по \ref{true}, что $V\equiv0$ (из формулы Даламбера с 0 начальными условиями) - единственное решение данной задачи.
\end{myproof}
\begin{myexe}
...
В этом разделе будем рассматривать решение волнового уравнения с заданными начальными условиями:
\[
\begin{cases}
...
В этом случае замена переменных, использованная для уравнения струны, уже не приводит к простому решению, поэтому будем искать функцию Грина $g\in C^\infty([0,\ \infty)),\D'(\R^3) $:
\begin{equation}
\label{eq:green-wave-3d}
\begin{cases}
...
\begin{enumerate}
    ...
    \item Возьмём предел при $\varepsilon \rightarrow 0 $
\end{enumerate}

1. Как и сказали выше, сделаем преобразование Фурье по $x$:
$\hat{g}(t,\xi) \in \mathcal{S}'(\R^3) $, тогда с учетом $\xi^2=|\xi|^2=\xi^2_1+\xi^2_2+\xi^2_3$, 
 наше уравнение перепишется в виде:
\[
 \dfrac{\p^2 \hat{g}(t,\xi)}{\p t^2} + c^2\xi^2\hat{g}(t,\xi) = 0
\]
Здесь нужно учитывать, что искомая функция Грина принадлежит $\D'(\R^3) $, а преобразование Фурье мы умеем делать 
только в $\mathcal{S}'(\R^3)$. Поэтому решение следует искать в пространстве темперированных распределений $\mathcal{S}'(\R^3)$. Полученное уравнение имеет знакомое решение:
\[
...
\[
\hat{g}\Big|_{t=0} = 0 \ \Rightarrow \ A = 0, \quad \dfrac{\p \hat{g}}{\p t} = Bc|\xi|\cos(c|\xi|t) \ \Rightarrow \ Bc|\xi| = \dfrac{1}{(2\pi)^{3/2}} \ \Rightarrow \hat{g}(t,\xi)=\dfrac{\sin(c|\xi|t)}{(2\pi)^{3/2}c|\xi|}
\]
Отсюда следует, что \(\hat{g}(t, \cdot)\in\Cinf(\R^3)\), на бесконечности она убывает, \(\hat{g}(t, \cdot)\in\mathcal{S}'(\R^3)\). Но она не интегрируема из-за того, что степень знаменателя должна быть больше размерности, но у нас она единица, поэтому \(\hat{g(t,\cdot)}\notin L_1(\R^3)\). Будем это исправлять. \\
2. Пусть \(\epsilon>0: \quad \hat{g}_\epsilon(t,\xi)=\hat{g}(t,\xi)e^{-\epsilon|\xi|}\). Тогда нетрудно догадаться: 
...
\[g_\epsilon(t,x)=\dfrac{1}{(2\pi)^{3/2}}\int\limits_{\R^3}e^{i\xi x}\dfrac{\sin(c|\xi|t)}{(2\pi)^{3/2}c|\xi|}e^{-\epsilon|\xi|}\dd \xi\underset{\xi=(\rho,\theta,\phi)}{=}\dfrac{1}{(2\pi)^3}\int\limits_{0}^{\infty}\int\limits_{0}^{\pi}\int\limits_{0}^{2\pi}e^{i|x|\rho\cos\theta}\dfrac{\sin(c\rho t)}{c\rho}e^{-\epsilon\rho}\rho^2\sin\theta\dd \rho\dd \theta \dd \phi=\]
...
\(\int\limits_{0}^{\infty}e^{-\epsilon\rho}\cos(\alpha\rho)\dd \rho=\operatorname{Re}\bigg(\int\limits_{0}^{\infty}e^{-\epsilon\rho}e^{i\alpha\rho}\dd \rho\bigg)=\operatorname{Re}\bigg(\int\limits_{0}^{\infty}e^{-(\epsilon-i\alpha)\rho}\dd \rho\bigg)=\operatorname{Re}\bigg(\dfrac{1}{\epsilon-i\alpha}\bigg)=\dfrac{\epsilon}{\epsilon^2+\alpha^2}\)\\
...
Остаётся проверить сходимость возникающих интегралов: при больших $t$ мы интегрируем функцию по большой сфере, мы знаем, что функция из класса Шварца убывает быстрее любой степени, поэтому на бесконечности получается что убывает тоже быстрее любой степени, в 0 тоже нет проблемы из-за того, что площадь порядка $t^2$, а $\phi$ ограниченная, поэтому особенности в 0 нет.
\\
Хочется сказать что работает:
\[\dfrac{\partial^2 G}{\partial t^2}-c^2\Delta G=\delta(t,x)\]
\begin{myproof}
    Эта запись носит формальный характер и отражает основную идею: \(G(t,x)=g(t, x)\theta(t)\).
    \[\dfrac{\partial G}{\partial t}=\dfrac{\partial g}{\partial t}\theta(t)+\underset{\small =0 \text{ из \ref{eq:green-wave-3d}}}{g\delta(t)}\]
    \[\dfrac{\partial^2 G}{\partial t^2}=\dfrac{\partial^2 g}{\partial t^2}\theta(t)+\underset{\small =\delta(t)\delta(x) \text{ из \ref{eq:green-wave-3d}}}{\dfrac{\partial g}{\partial t}\delta(t)}\]
...
\begin{mytheorem} \label{thm:wave-3d-solution}
...
\begin{myremark}
    Именно условие $G\Big|_{t<0}=0$ выделяет единственность решение задачи:
    \[\dfrac{\p^2 G}{\p t^2} - c^2\Delta G = \delta(t,x)\]
\end{myremark}
\begin{myproof}
    Сразу выпишем первое и второе слагаемые:
...
Третье слагаемое рассмотрим подробнее:
\[
...
Перейдём в сферические координаты следующим образом $y = x+(cs,\omega,\psi)$ и раскроем $\dd S$:
\[
...
Записав, что $\dd y=c^2s^2\sin\omega\dd \omega\dd \psi \dd (cs)$, получим:
\[
...
\item Пусть $h\in C(\R^n)$, тогда производная от интеграла по шару по радиусу:
    ...
    \]
Эта формула следует из перехода к сферическим координатам:
\[
\int\limits_{B_r(0)} h(x)\,\dd x
=\int_0^r\left(\int\limits_{S_\rho(0)} h(x)\,\dd S(x)\right)\dd\rho.
\]
Дифференцируя по верхнему пределу $r$, получаем требуемое равенство.
...
\begin{mylemma}\label{lem:spherical-mean}
...
\end{mylemma}
\begin{myproof}
Выпишем необходимые производные, используя приведённые выше равенства:
...
\
Докажем теорему \ref{thm:wave-3d-solution} для $u_2$ при $r=ct$.

Запишем ранееё полученное выражение для $u_2$ и воспользуемся усреднением, которое только что изучили:
\[
...
Подметим замечательныйе факт, что при деформации (сжатии) сферы,  среднее значение стремится к значению подынтегральной функции в центре(аналогично теореме о среднем в электростатике):
\[
...
В скобках получили выражение из леммы \ref{lem:spherical-mean}:
\[
...
\begin{myexe}
    ...
    являются решениями следующих систем:
    \[
\begin{cases}
...
\begin{mylemma}\label{lem:moving-ball-derivative}
...
\begin{myproof}
    Для доказательтва напишем полную производную по времени и будем пользоваться леммой \ref{lem:moving-ball-derivative}:
\[
...
Рассмотрим первый интеграл подробнее и вспомним, что $u$ является решением волнового уравнения:
\[
...
\begin{myremark}
    Если бы $$\int\limits_{\R^n}\skobka{\skobka{\dfrac{\p u}{\p t}}^2 + c^2\skobka{\nabla u}^2}\dd x \leq \infty, $$ то \[\Rightarrow \int\limits_{\R^n}\skobka{\skobka{\dfrac{\p u}{\p t}}^2 + c^2\skobka{\nabla u}^2}\dd x = \operatorname{const}\] --
     Это равенство имеет смысл закона сохранения энергии: функция $\Psi$ играет роль энергии. Поскольку граница шара движется со скоростью $c$, влияние извне не успевает проникнуть внутрь области зависимости.

\end{myremark}
Теперь для теоремы \ref{thm:wave-3d-solution} докажем единственность:
...
Для определённости выберем  точку $(t_0,x_0)$ и посмотрим на  функцию:
\[
...
\]
 ...
 \end{enumerate}
 Ясно, что если $t<\dfrac{d_1}{c} \Rightarrow u(t,x)=0$, аналогично $t>\dfrac{d_2}{c}\Rightarrow u(t,x)=0$. 
\begin{center}
\begin{tikzpicture}[scale=0.65]
    ...
    % область K
    \fill[gray!15] (0,0) ellipse (3 and 1.8);
    \draw[thick] (0,0) ellipse (3 and 1.8);

    % внешние фронты
    \draw[blue, thick] (0,0) ellipse (4.0 and 2.6);
    \draw[blue, thick] (0,0) ellipse (5.2 and 3.4);

    % метка K
    \node at (0,0) {\Huge $K$};

    % точки на границе K
    \coordinate (A) at (-2.9,0.25);
    \coordinate (B) at (2.65,-0.75);

    % точка x
    \coordinate (X) at (8.2,-4.2);
    ...
    % подписи
    \node[below right] at (X) {\Huge $x$};
    \node[right] at (4.6,-1.5) {\Huge $d_1$};
    \node[below] at (4,-2.6) {\Huge $d_2$};

\end{tikzpicture}
\end{center}


\textbf{Передним фронтом волны} называется следующее множество:
\[
...
\begin{myremark}
    Краевые условия необязательно должны быть нулевыми: произвольные условия можно свести к однородным добавлением подходящей функции.
\end{myremark}
\begin{myremark}
    Краевые условия могут быть записаны в нескольких стандартных формах:\\
    условие Дирихле: $y(a)=A, \ y(b)=B$\\
    условие Неймана: $y'(a)=A, \ y'(b)=B$\\
В самом общем виде можно рассматривать смешанные условия:
\[\begin{cases}
...
\begin{mytheorem}
    Предположим, что у однородной задачи \eqref{eq:sturm-bvp} ($f\equiv0$), существует только тривиальное решение ($y\equiv0$), тогда:
    \[\forall f \in C[a,b]\quad \exists!\,y\in C^2[a,b],\] которое является решением задачи \eqref{eq:sturm-bvp}. 
\end{mytheorem}
...
У него есть фундаментальная система решений (ФСР), состоящая из двух линейно независимых 
...
При таком построении $\phi_1\not\equiv0$, так как $\psi_1$ и $\psi_2$ линейно независимы.
...
Они действительно линейно независимы. Если бы $\phi_2(x) = \gamma\phi_1(x)\Rightarrow \phi_1$ --- нетривиальное решение решение исходной однородной задачи \eqref{eq:sturm-bvp}. Но по условию теоремы требуем, чтобы там было только тривиальное решение.
...
\[\begin{cases}
    ...
    y(a)=y(b)=0
\end{cases}\] 
Искать решение мы будем знакомым способом -- \textit{методом вариации произвольной постоянной}:
\[
...
Тем самым существование решения доказано. Осталось доказать его единственность.
Пусть существуют два решения $y_1$ и $y_2$, но тогда $(y_1 - y_2) $ -- это решение однородного уравнения, которое обязано быть тривиальным, по условию теоремы. Получаем $y_1 \equiv y_2$.

\end{myproof}


\begin{mydef}
    Функция Грина для задачи \eqref{eq:sturm-bvp} --  $G(s,t), \ G\in C\skobka{[a,b]\times[a,b]}$ -- ядро интегрального оператора. Другими словами:
\[
...
\begin{mydef}
    Функцию Грина для задачи \eqref{eq:sturm-bvp}, можно записать еще и следующим образом:
    \[
\begin{cases}
...
Вычисляя первую производную, получим:
\[G'_s(s,t) = 
\begin{cases}
...
\end{cases}
\]
Поскольку производная рассматривается в обобщённом смысле, необходимо проверить скачок классической производной:
...
\begin{mytheorem}
    Предположим, что у однородной задачи \eqref{eq:sturm-bvp}, существует нетривиальное решение $y_0,$ тогда у неоднородной задачи либо нет решений, либо их бесконечно много
\end{mytheorem}


\begin{myproof}
    Если существует $y_1$ -- нетривиальное решение неоднородной задачи \eqref{eq:sturm-bvp}, то $y(x)=y_1(x)+Cy_0(x)$ -- тоже решение.
\end{myproof}
...
Посмотрим на следующие интегралы:
    ...
    Следовательно, если решение $y$ существует, то $\int\limits_{a}^{b}f(x)\phi(x)\dd x = 0$ -- \textit{необходимое условие разрешимости}
\end{enumerate}
\end{myexample}
\begin{myremark}
    \textit{Линейная алгебра за 5 минут}\\
    Рассматриваем матрицу $A\in Mat(n\times n)$ и уравнение:
    \[A\vec{x}-\lambda\vec{x} = \vec{b}, \quad \vec{b},\vec{x}\in\C^n,\] тогда
    \begin{enumerate}
        \item Если $\lambda \not\in \operatorname{spec} A \Rightarrow \forall \vec{b} \in \C^n \ \exists! \vec{x}\in\C^n: A\vec{x}-\lambda\vec{x} = \vec{b}$ и надо просто посчитать такой определитель:
        \[\operatorname{det}(A - \lambda I)\]
        \item Если $\lambda \in \operatorname{spec} A \Rightarrow$ решение существует при условии разрешимости на правую часть, если существует, то их бесконечно много.
        \\
        Если имеется базис из собственных векторов, $A\vec{u_k}=\lambda \vec{u_k}$, то мы можем разложить по этому базису:
        \[\vec{x} = \sum_{k=1}^n x_k \vec{u_k}\]
        \[A\vec{x}=\sum_{k=1}^n x_k \lambda_k \vec{u_k}\]
    \end{enumerate}
\end{myremark}
...
Здесь $y\in C^2([a,b])$ --- неизвестная функция, а $\lambda\in\R$ --- это спектральный параметр.\\
...
\begin{myremark}
    Может быть такое, что $p(x)>0 \ \ \forall x\in(a,b)$ и $p(a) = p(b) = 0$, тогда на $y(x)$ нет никаких краевых условий.
\end{myremark}
...
Стандартные аксиомы скалярного произведения выполняются. Полученное пространство является \textbf{предгильбертовым}; для гильбертова пространства дополнительно требуется полнота. 
\begin{myremark}
...
Точки в записи оператора обозначают место подстановки функции, на которую действует оператор: 
\[
\LL y = \dfrac{-(py')'+qy}{r}
\]
Очевидно, что $\LL$ -- это линейный оператор. Но он задан на бесконечномерном пространстве, поэтому его область определения:
\[
...
\begin{myproof}
    Рассмотрим левую часть и применим интегрирование по частям, учитывая, что $f(a) = f(b) = g(a) = g(b) = 0$:
    \[
...
\begin{myremark}
    ...
    Но неинтегральные слагаемые будут обнуляться уже за счет функции $p$, а не благодаря функциям $f$ и $g$.
\end{myremark}
...
\begin{mytheorem}\label{thm:sturm-simple-spectrum}
    Спектр оператора $\LL$ -- однократный, то есть собственные подпространства одномерны $\LL y = \lambda y$. Собственные функции, соответствующие различным собственным числам, ортогональны.
\end{mytheorem}
\begin{myproof}
    \begin{enumerate}
        \item Сначала докажем первое утверждение. \\Пусть у нас имеется собственное число -- $\lambda$, которому 
        соответствуют две собственные функции $U,V$, удовлетворяющие однородной задаче Штурма-Лиувилля:
        \[\begin{cases}
            (pu')'=(q-\lambda r)u\\
            (pv')'=(q-\lambda r )v
        \end{cases}\]
    Рассмотрим комбинацию, $p(u'v-uv')$, которую продифференцируем, тогда:
    \[\skobka{p(u'v-uv')}'=(pu'v)'-(puv')'=(pu')'v - u(pv')'  = u(q-\lambda r)v - (q-\lambda r)uv = 0 
    \Rightarrow\]\[\Rightarrow p(u'v-uv') = const\ \text{на } [a,b]\]
    
    Чтобы определить эту константу, рассмотрим значение введённого выше соотношения в точке $a$. Так как $u(a)=v(a)=0$, то:
    \[p(u'v-uv')\Big|_{x=a}=0\Rightarrow p(u'v-uv')=0 \Rightarrow u'v=uv' \Rightarrow v=cu, \ \text{где $c$ -- константа}
    \] 
...
\end{cases} \Rightarrow \lambda \neq \mu \Rightarrow \int\limits_{a}^{b}u(x)v(x)r(x)\dd x = 0 \]
    ...
    Тогда для $\lambda\neq\mu$ это равенство выполняется только в том случае, когда $(u,v)_r=0$, то есть собственные функции ортогональны.
...
\begin{myremark}
    Если $p(a) = p(b) = 0$, то всё показанное выше работает, в том числе и теорема \ref{thm:sturm-simple-spectrum}.
    \[p(u'v-uv')\Big|_{x=a}=0 \Rightarrow p(u'v-uv')=0 \Rightarrow u'v-uv'=0 \ \text{в }[a,b]\]
\end{myremark}

\begin{myremark}
    Выше все функции считались вещественнозначными. Можно также рассмотреть комплекснозначный случай, оставив коэффициенты $p,q,r$ вещественными. 
    $y, \lambda$ -- комплексные. Тогда $\LL$ -- это эрмитов оператор, а не симметричный, но все доказательства переносятся на этот случай аналогичным образом.
...
\begin{myproof}
    Докажем от противного. Пусть существует $y(x_k) = 0, \ k = 1,2,\dots$. По теореме Больцано—Вейерштрасса существует подпоследовательность $\{x_{n_{\alpha}}\}$ которая сходится к $x_0 \in [a,b]$. 
    ...
    \[\begin{cases}
        y(x_0)=0\\
        y'(x_0)=0
    \end{cases} \Rightarrow \text{по теореме существования и единственности решения} \Rightarrow y \equiv 0
    \]
\end{myproof}
...
Тогда, между любыми двумя нулями функции $u$ лежит хотя бы один ноль функции $v$.
\end{mytheorem}

\begin{myproof}
    Пусть $x_1,x_2$ — два соседних нуля функции $u$. Не умаляя общности, будем считать, что $u(x)>0 \ \forall x\in(x_1,x_2) $. Отсюда следует, что $u'(x_1)>0$ и $u'(x_2)<0$. Покажем, что на интервале $(x_1,x_2)$ функция $v$ имеет хотя бы один ноль.
    Докажем от противного. Пусть $v(x)\neq0 \ \forall x\in(x_1,x_2) $ --- тогда, не умаляя общности, $v(x)>0 \ \forall x\in(x_1,x_2), \ v(x_1)\geq0, \ v(x_2)\geq 0$. Рассмотрим вспомогательную непрерывную функцию $F(x) = p(x)u'(x)v(x) - p(x)u(x)v'(x)$ и вычислим её производную:
    \[
...
\]
Получаем противоречие, поскольку $F$ -- строго возрастает! Наше начальное предположение о том, что $v(x)\neq0$, неверно и $v(x)=0$ на $(x_1,x_2) $.
\end{myproof}
...
\begin{myproof}
    $\mathbf{(\impliedby)}$ Действительно,  $\psi(\cdot,\lambda)$ --- это нетривиальное решение задачи Штурма-Лиувилля.
...
Посмотрим на пару функций $y(x)$ и $y'(a)\psi(x,\lambda)$ --- они обе являются решениями нашей задачи, в точке $a$ они обе обнуляются, а их производные в точке $a$ равны $y'(a)$. Тогда по теореме о единственности мы понимаем, что $y(x)\equiv y'(a)\psi(x,\lambda)$ --- отсюда ясно, что $\psi(b,\lambda) = 0$.
\end{myproof}
...
\begin{myproof}
    Снова докажем от противного. Пусть $c\in(a,b)$ --- это первый нуль функции $\psi: \psi(a,\lambda)=0= \psi(c,\lambda)$. Не умаляя общности, $\psi(x,\lambda)>0 \ \forall x\in(a,c)$ и мы знаем $\lambda\leq \min\limits_{x\in[a,b]}\dfrac{q(x)}{r(x)} \ \forall x\in[a,b]$, то есть $q(x) - \lambda r(x)\geq0 \ \forall x\in[a,b]$. Тогда:
\[
    (p\psi')' = \underbrace{(\lambda r - q)}_{\geq0}\underbrace{\psi}_{>0} \geq 0 \Rightarrow p(x)\psi'(x) \geq p(a)\psi'(a) = 1 \ \forall x\in[a,c] \]
    Отсюда следует, что $\psi'(x)>0 \ \ \forall z\in[a,c] \implies \psi(c)>0$ -- получили противоречие.
\end{myproof}


\begin{myremark}
    Пусть у функции $\psi$ есть нули на $[a,b]$, обозначим их через $z_j(\lambda)$ и расположим в порядке возрастания, тогда:
...
\begin{myproof}
    Обозначим $z_0(\lambda)\equiv a$. Докажем утверждение индукцией по $j$.
    \begin{enumerate}
    \item База: $j = 0$ 
    ...
    По теореме Штурма, между любыми двумя нулями $\psi(x,\lambda_0)$ лежит по крайней мере один ноль $\psi(x,\lambda_1) $, то есть:
    \[
    z_j(\lambda_0) < z_k(\lambda_1) < z_{j+1}(\lambda_0)
    \]
    Воспользуемся предположением индукции, получаем, что $k\geq j+1$ и, соответственно, $z_{j+1}(\lambda_1)\leq z_k(\lambda_1)<z_{j+1}(\lambda_0)$ -- что и требовалось показать.
\end{enumerate}
\end{myproof}
...
\begin{myremark}
    Пусть задан отрезок $[\mu_1, \mu_2]\in \R$, тогда у задачи Штурма-Лиувилля конечное число собственных чисел $\lambda \in [\mu_1, \mu_2]$.  Если бы это было не так, то у функции $\psi(x,\lambda)$ было бы бесконечно много нулей, что является противоречием.
\end{myremark}
...
\begin{mytheorem}
    ...
    Если $\lambda_n\to \infty$, то $y_n$ имеет $(n-1) $ нулей на $[a,b]$ и $y_n$ образуют ортогональную полную систему в $L_2([a,b])$
\end{mytheorem}
...
$y=C_1\sin(\sqrt{\lambda}x)+C_2\cos(\sqrt{\lambda}x)$. Из граничных условий следует, что $C_2=0$ и $\lambda_n=n^2;\  n\in \Z$. Тогда собственные функции имеют вид:
...
будет иметь вид:
\[\psi(x,\lambda) = \dfrac{\sin(\sqrt{\lambda}x)}{\sqrt{\lambda}}.\]
Эта функция удовлетворяет условиям $\psi(0,\lambda)=0$ и $\psi'(0,\lambda)=1$. Собственные значения исходной задачи Дирихле определяются дополнительным условием $\psi(\pi,\lambda)=0$, откуда $\sqrt{\lambda_n}=n$ и $\lambda_n=n^2$.
...
Выполнено следующее: $\lambda_n = n^2 + \bigo{n\to\infty}{1}$   
\end{myexe}


\section{Неоднородная конечная струна}
В этом разделе рассмотрим неоднородную струну на $[a,b]$, с некоторыми условиями:
\[
\begin{cases}
...
\end{cases}
\] Сформулируем общий метод решения, предполагая, что все входящие функции достаточно гладкие для выполняемых преобразований.
...
Имеется бесконечная последовательность положительных собственных чисел $\lambda_n \xrightarrow[n\to \infty] \ +\infty$ и соответствующих им собственных функций $y_n$, образующих полную систему. Другими словами, для любого $f\in\L_2([a,b])$ соответствующий ряд сходится в $L_2$, то есть $$\int\limits_{a}^{b}\Bigg|f(x)-\sum_{n=1}^{k}f_n(t)y_n(x)\Bigg|^2\dd x \xrightarrow{k \to \infty} 0 \quad \text{ в }\L_2([a,b])$$ или же:
\[g\in\L_2([a,b]): g \perp y_n \ \ \forall n \Rightarrow g\equiv0\]
...
\begin{myremark}
Чем глаже функция, тем быстрее убывают её коэффициенты Фурье. Например,
если
\[
f\in C^n[a,b],
\]
то существует такое число $M\in\mathbb{N}$, что при всех $m>M$
\[
|f_m|<\dfrac{C}{m^n}
\]
\end{myremark}
Ищем решение нашей задачи в виде ряда:
\[
u(x,t)=\sum_{n=1}^{\infty} u_n(t)y_n(x)
\]
...
=
\sum_{n=1}^{\infty} \ddot u_n(t)y_n(x)
\]

Также, используя задачу на собственные значения, получаем:
\[
-\left(
p(x)\sum_{n=1}^{\infty}u_n(t)y_n'(x)
\right)'
=
\sum_{n=1}^{\infty}u_n(t)\lambda_n y_n(x)
\]

Подставляя разложение в исходное уравнение, имеем:
\[
\sum_{n=1}^{\infty}\ddot u_n(t)y_n(x)
=
-\sum_{n=1}^{\infty}u_n(t)\lambda_n y_n(x)
+
\sum_{n=1}^{\infty}y_n(x)f_n(t)
\]
...
y_n(x)
\left(
\ddot u_n(t)+\lambda_n u_n(t)-f_n(t)
\right)
=0
\]

В силу полноты системы $\{y_n\}$, получаем для каждого $n\in\mathbb{N}$
обыкновенное дифференциальное уравнение:
\[
\ddot u_n(t)+\lambda_n u_n(t)-f_n(t)=0.
\]

Начальные условия также раскладываются по собственных функций:
\[
\sum_{n=1}^{\infty}u_n(0)y_n(x)
=
\sum_{n=1}^{\infty}\varphi_n y_n(x)
\Rightarrow
u_n(0)=\varphi_n
\]
...
Аналогично:
\[
\sum_{n=1}^{\infty}\dot u_n(0)y_n(x)
=
\sum_{n=1}^{\infty}\psi_n y_n(x)
\Rightarrow
\dot u_n(0)=\psi_n
\]

Таким образом, для каждого $n$ нужно решить задачу Коши:
\[
\begin{cases}
\ddot u_n(t)+\lambda_n u_n(t)=f_n(t)
\\[0.5em]
...
Рассмотрим вспомогательную задачу для $ \lambda>0$:
\[
\begin{cases}
\ddot g(t)+\lambda g(t)=0
\\[0.5em]
g(0)=0
\\[0.5em]
\dot g(0)=1
...
Её решение мы знаем:
\[
g(t)=\dfrac{1}{\sqrt{\lambda}}\sin(\sqrt{\lambda}\,t)
\]

Тогда решение задачи:
\[
\begin{cases}
\ddot u(t)+\lambda u(t)=f(t)
\\[0.5em]
u(0)=\varphi
\\[0.5em]
\dot u(0)=\psi
...
g(t)\psi
+
\int_0^t g(t-\tau)f(\tau)\,\dd \tau
\]

Заметим, что:
\[
\dot g(t)=\cos(\sqrt{\lambda}\,t),
\]
тогда получаем для коэффициентов $u_n(t)$:
\[
u_n(t)
=
\cos(\sqrt{\lambda_n}\,t)\varphi_n
+
\dfrac{\sin(\sqrt{\lambda_n}\,t)}{\sqrt{\lambda_n}}\psi_n
+
\int_0^t
\dfrac{\sin(\sqrt{\lambda_n}(t-\tau))}{\sqrt{\lambda_n}}
f_n(\tau)\,\dd \tau
\]
...
\subsection{Пример: однородная струна}

Рассмотрим частный случай однородной струны длины $\ell$ с закреплёнными
концами. Пусть струну оттянули и отпустили, то есть начальная скорость
равна нулю. Тогда:
\[
\begin{cases}
\dfrac{\partial^2 u(x,t)}{\partial t^2}
=
c^2\dfrac{\partial^2 u(x,t)}{\partial x^2}
\\[1em]
u(0,t)=u(\ell,t)=0
\\[0.5em]
u(x,0)=\varphi(x)
\\[0.5em]
\dfrac{\partial u(x,t)}{\partial t}\bigg|_{t=0}=0
\end{cases}
\]

Соответствующая задача на собственные значения:
\[
\begin{cases}
-\left(c^2y'(x)\right)'=\lambda y(x)
\\[0.5em]
y(0)=y(\ell)=0
\end{cases}
\]

Так как $c=\operatorname{const}$, получим:
\[
-c^2y''(x)=\lambda y(x)
\]
...
\[
\lambda_n=\dfrac{n^2\pi^2c^2}{\ell^2},
\qquad n=1,2,\ldots
\]

Собственные функции имеют вид:
\[
y_n(x)=\sin\left(\dfrac{n\pi x}{\ell}\right)
\]

Так как начальная скорость равна нулю и внешняя сила отсутствует, получаем:
\[
u_n(t)
=
\cos\left(\dfrac{n\pi c}{\ell}t\right)\varphi_n
\]
...
\sum_{n=1}^{\infty}
\varphi_n
\cos\left(\dfrac{n\pi c}{\ell}t\right)
\sin\left(\dfrac{n\pi x}{\ell}\right)
\]

Здесь $c$~--- скорость распространения механических волн в струне, то есть
...
\subsection{Вывод}
Рассмотрим тепловой поток $\vec{W}(t,x)=-k\nabla u,$ где $u(t,x)$ -- температура. Введём физические параметры: $$c,\rho, k = const,$$ где $c$ -- удельная теплоёмкость вещества, $\rho$ -- плотность вещества. 

Сейчас рассмотрим область $\Omega \in \R^n$ и запишем изменение тепловой
энергии в промежутке времени от $t_1$ до $t_2$. Энергия представляется в виде $\int c\rho u(t,x)\dd x$, тогда её изменение принимает вид:
...
\[\dfrac{\p u}{\p t}+a \Delta u = 0(=F(t,x)),\] где $a\dfrac{k}{c\rho}$, $F(t,x)$ --  возможный источник тепла.
\begin{mydef}
...
Далее будем считать, что $a=1$, тогда наша задача такая:
\[
\begin{cases}
...
\begin{cases}
 ...
 \int\limits_{\R^n}\phi(x)e^{-ix\xi}\dd x
\end{cases}
\]
...
\begin{mylemma}
    Пусть $t>0$, тогда $\mathcal{F}[e^{-\xi^2t}](x) = \mathcal{F}^{-1}[e^{-tx^2}]=(2\pi)^{-n/2}e^{-\xi^2/(4t)}$
\end{mylemma}
\begin{myproof}
    ...
    Нам достаточно рассмотреть одномерный случай $n=1$, тогда рассмотрим следующую функцию $f(x)=e^{-tx^2}$, получим:
...
\begin{myproof}
   ...
   После вычисления производных получаем $\dfrac{\p u}{\p t} - \Delta u = 0$. \\
   Воспользуемся леммой о $\delta$ -- образных последовательностях, которая гласит, что если $f\in C(\R^n)$ и $f$ ограничена, то:
\begin{enumerate}
...
\begin{enumerate}
    ...
    Отсюда видно, что первое условие леммы о $\delta$ -- образных последовательностях выполнено.
    ...
    Далее сведём вычисления к интегралу Пуассона, тогда:
...
\begin{enumerate}
    ...
    Как только $t = \epsilon>0$, решение сразу становится бесконечно гладким
    ...
    Для минимума доказательство аналогично, только везде надо будет поменять знак неравенств.
    ...
    В этом случае необходимо уточнить условия. Нужно требовать $\psi_0\in\L_2(\R^n)$ -- чтобы была сходимость и $\psi(t_i)\in\L_2(\R^n)$ -- чтобы сохранялась норма. Тогда $\psi$ -- это решение уравнения Шрёдингера, так как $\psi$ -- это просто $u$ при $t \to -it$. 
\end{enumerate}
...
\begin{mytheorem}
    ...
    Тогда  существует единственное решение $u\in C^{1,2}((0,\infty)\times\R^n)$, непрерывное и ограниченное в $[0,\infty)\times\R^n$ и удовлетворяющеё системе:
\[
\begin{cases}
...
\begin{myproof}
    ...
    \[\begin{cases}
        ...
        u_2\Big|_{t=0} = 0
    \end{cases}\]
    причём необходимо проверить, что $u_2$ -- это решение такой задачи:
...
Перепишем $u_2$ в виде свертки:
\[u_2(t,x) = \int\limits_{0}^{t}\int\limits_{\R^n}g(\tau,y)f(t-\tau, x-y)\dd y\dd \tau\]
И посчитаем производные от $u_2$:
\[\dfrac{\p u_2}{\p t} = \int\limits_{\R^n}g(t,y)f(0, x-y)\dd y + \int\limits_{0}^{t}\int\limits_{\R^n}g(\tau,y)\dfrac{\p f}{\p t}(t-\tau,x-y)\dd y\dd \tau\]
\[\dfrac{\p u_2}{\p x_i} = \int\limits_{0}^{t}\int\limits_{\R^n}g(\tau,y)\dfrac{\p f}{\p x_i}(t-\tau, x-y)\dd y\dd \tau\]
\[\dfrac{\p^2 u_2}{\p x_i \p x_j} = \int\limits_{0}^{t}\int\limits_{\R^n}g(\tau,y)\dfrac{\p^2 f}{\p x_i \p x_j}(t-\tau, x-y)\dd y\dd \tau\]
Итак:
\[\dfrac{\p u_2}{\p t} - \Delta_x u_2 = \int\limits_{0}^{t}\int\limits_{\R^n}g(\tau,y)\left(\dfrac{\p f}{\p t}(t-\tau, x-y) - \Delta_x f(t-\tau, x-y)\right)\dd y\dd \tau+\]\[+\int\limits_{\R^n}g(t,y)f(0, x-y)\dd y :=I_1+I_2\]
    Рассмотрим $I_1$, взяв $\epsilon>0$ и разобьём по промежутку:
    \[
...
Второй интеграл надо расписать по производным и брать по частям:
\[
    ...
    \]
    Подставим полученные выражения в исходную задачу:
     \[
...
\begin{myexe}
    \begin{enumerate}
        \item Пусть задана функция $v\in C^{1,2}(Q)\cap C(\overline{Q})$, максимум которой случился в некоторой точке $(t_1,x_1) $:
\[
...
Для этого упражнения приведём несколько указаний:\\
Введём обозначения $M = \underset{\overline{Q}}{\operatorname{max}}\,u$ и $M_1 = \underset{{\p' Q}}{\operatorname{max}}\,u < M$ и  тогда рассмотрим функцию:
\[
v(t,x) = u(t,x) + \dfrac{1}{2(\operatorname{diam}(\Omega))^2}\skobka{M-M_1}|x-x_0|^2,
\]где $\max\limits_{\overline{Q}}u(t,x) = u(t_0,x_0);\,(t_0,x_0)\not\in\p'Q$
Достаточно проверить два утверждения:
\vspace{0.5cm}
\begin{itemize}
    ...
    Для доказательства единственности достаточно применить принцип сравнения к разности двух решений. Если $u_1$ и $u_2$ имеют одинаковые начальные данные, то $w=u_1-u_2$ удовлетворяет однородному уравнению теплопроводности и $w|_{t=0}=0$. Применяя принцип максимума к $w$ и $-w$, получаем $w\equiv0$, следовательно, $u_1\equiv u_2$.
    \end{enumerate}
\end{myexe}
...
\end{cases}\]
Здесь предполагается, что $p\in C^1([a,b]), \ p(x)>0 \ \forall x\in[a,b]$, $q\in C([a,b])$.\\
Рассмотрим задачу Штурма-Лиувилля, которая будет связана с нашим уравнением теплопроводности. Получаем следующую задачу:
\[
\begin{cases}
    \LL y=-(py')' + qy = \lambda y\\
    y(a) = y(b) = 0
\end{cases}
\]
Обозначим $\lambda_n$ -- это собственные числа, а $y_n$ -- это собственные функции. Тогда мы можем разложить по $y_n$:
\[
...
\begin{cases}
    ...
    u_n(0) = \phi_n
\end{cases}
\] А для такой за   дачи мы уже знаем решение:
...
\[\begin{cases}
    ...
    u\Big|_{t=0} = \phi(x), \ \ u\Big|_{x=a} = u\Big|_{x = b} = 0
\end{cases}\] Решение выражается через функцию Грина для $\LL$:
\[
...
Следует отметить сходимость ряда: она обеспечивается экспоненциальным множителем $\lambda_n\xrightarrow[n\to\infty]{}\infty\propto  n^2$.\\
...
\]
Проверим сходимость на произвольной области $\D'(\Omega)$. Пусть $\phi\in C^{\infty}_0(a,b)$,тогда ряд $\phi(x) = \SUMN\ \phi_ny_n(x)$ сходится в $C^{\infty}_0(a,b)$, тогда:
...
\[\Delta g =\delta (x)\]
Здесь мы будем искать $g$, которая сферически симметрична, тогда просто переобозначим ее:
\[g(x)=f(r), \quad |x|=r=\sqrt{x^2_1+\cdots +x^2_n}\]
Очевидно, что если окружить точку 0 шаром, то $\Delta f = 0$ вне $\forall B_n(0)$, тогда:
...
\[\Delta f = f''(r)+f'(r)\left(\dfrac{n-1}{r}\right)\Rightarrow -f''(r)=f'(r)\dfrac{n-1}{r}\Rightarrow f(r)=\begin{cases}
    c\ln(r), \ \ n=2\\
...
\begin{myproof}
    ...
    \[
    \int\limits_{\Omega}\operatorname{div}\Vec{w} \dd x= \int\limits_{\p\Omega}\langle \vec{w},\vec{\nu}\rangle\dd S
    \]
...
И $\langle \vec{w},\vec{\nu}\rangle = u\dfrac{\p v}{\p \nu} - v\dfrac{\p u}{\p \nu}$, откуда сразу следует вторая формула Грина.
\end{myproof}


\begin{mytheorem}\label{th:green-formula}
    Пусть $\Omega\subset\R^n$ — ограниченная область с кусочно-гладкой границей, а $u\in C^2(\Omega)\cap C^1(\overline{\Omega})$. Тогда для любого $x\in\Omega$ имеем:
...
\begin{myproof}
    Возьмём $\varepsilon > 0$ и рассмотрим область $\Omega_\epsilon := \Omega\setminus \overline{B_\epsilon(x)}= \Omega \setminus \{y: |x-y| < \epsilon\},$ чтобы убрать проблемы функции Грина $g_0(x-y)$ в точке $y=x$. Тогда положив $v(y) = g_0(x-y)$, применим вторую формулу Грина:
\[
...
\]
\[
\int\limits_{\p B_\epsilon}u(y)\dfrac{\p g_0(x-y)}{\p\nu}\dd S(y) = - \int\limits_{\p B_\epsilon}u(y)\dfrac{\p g_0(x-y)}{\p r}\dd S(y)=-\int\limits_{\p B_\epsilon(x)}u(y)\dfrac{1}{\kappa_n\epsilon^{n-1}}\dd S(y) \to \]\[\xrightarrow[\epsilon\to0]{}-u(x)
\]
\end{myproof}

Отсюда вытекает следующее следствие: если рассмотреть функцию $u\in \D(\R^n),$ тогда:
\[
...
\begin{myremark}
    Для функции Грина для случая $n=1$, то она будет иметь вид:
\[g_0(x) = \dfrac{1}{2}|x|
\]
...
\begin{myexample}
    \begin{enumerate}
        ...
        \item $\ln|x|$ -- гармоническая в $\R^3 \setminus \{0\}$
...
\begin{enumerate}
    \item Для гармонической функции $u\in C^2(\Omega)\cap C^1(\overline{\Omega})$ справедлива следующая формула:
    \[
    u(x) = \int\limits_{\p \Omega}\skobka{u(y)\dfrac{\p g_0(x-y)}{\p \nu} - g_0(x-y)\dfrac{\p u(y)}{\p \nu}}\dd S(y),
    \] то есть значение функции в точке восстанавливается по её значению на границе и значению её нормальной производной.
...
\begin{mytheorem}
   ...
   Рассмотрим гармоническую в области $\Omega\subset\R^n$ функцию $u$, и шар $\overline{B_R(x)}\subset\Omega$. Тогда верно усреднение по сфере:
\[
...
\[\int\limits_0^R \varkappa_n r^{n-1}u(x)\dd r = \int\limits_0^R\int\limits_{|x-y|=r}u(y)\dd S(y)\dd r\Rightarrow \dfrac{\varkappa_n R^n}{n}u(x) = \int\limits_{B_R(x)}u(y)\dd S(y)\]
\end{myproof}
...
\begin{myproof}
    Рассмотрим функцию $w = u-v$ (она подходит под условия предыдущей теоремы), она гармоническая в $\Omega$ и $w\geq 0$ на границе, тогда по принципу максимума $w\geq 0$ в $\Omega$, откуда $u\geq v$ в $\Omega$.
\end{myproof}
...
\\
Давайте вспомним полученную выше формулу для гармонической функции:
\[u(x) = \dfrac{n}{\kappa_nR^n}\int\limits_{B_R(x)}u(z)\dd z\]
и рассмотрим произвольные точки $x,y$ внутри шара $B_R(x_0)$. Для таких точек из неравенства треугольника следует:
\[|x-y|\leq|x-x_0|+|y-x_0|<2R\]
Далее рассмотрим шар $B_R(x)$ и точку $z$, внутри него. Запишем внимательно аналогичное неравенство треугольника:
\[|z-y|\leq |z-x|+|x-y|\leq R+2R=3R\]
Откуда получаем соотношение на вложенные друг в друга шары:
\[B_R(x) \subset B_{3R}(y)\subset B_{4R}(x_0)\subset\Omega\]
Так как точка $y \in B_{R}(x_0)$ то справедливо соотношение на знакую нам формулу(это следует из вложенности шаров):
\[\int\limits_{B_R(x)}u(z)\dd z \leq \int\limits_{B_{3R}(y)}u(z)\dd z \Rightarrow u(x)\leq \dfrac{n}{\kappa_nR^n}\int\limits_{B_{3R}(y)}u(z)\dd z\]
Теперь запишем формулу для $u(y)$:
\[u(y)= \dfrac{n}{\kappa_nR^n}\int\limits_{B_{3R}(y)}u(z)\dd z\]
Подставим в неравенсвто выше это определение и получим:
\[u(x)\leq \Big|\dfrac{B_{3R}}{B_{R}}\Big|u(y)\Rightarrow u(x)\leq 3^n u(y)\]
Так как точки $x,y$ выбирались произвольно, то одну возьмём за $\max$, а другую за $\min$ и получим нужное нам итоговое неравенство:
\[\max\limits_{B_R(x_0)}u \leq 3^n\min\limits_{B_R(x_0)}u\]
\end{myexe}
...
\\ Здесь мы говорим что решение существует, этот факт мы покажем чуть позже, а сейчас мы показали единственность решения.
\end{myproof}
...
\begin{myremark}
    Условие разрешимости задачи Неймана -- это не только необходимое, но и достаточное условие для существование решения задачи Неймана.
\end{myremark}
...
\begin{mylemma}\label{lem:exterior-harmonic-decay}
...
\begin{myproof}
Доказательство будет дано ниже, в \hyperref[proof:exterior-harmonic-decay]{соответствующем доказательстве}.
\end{myproof}
...
\begin{myproof}
Возьмём $R>0$ настолько большим, что:
\[
...
\begin{myremark}
Разрешимость соответствующих задач Неймана и Дирихле будет обсуждаться далее.
\end{myremark}
...
\]
Очевидно, что это регулярная обобщённая функция и $\C^{\infty}$ -- гладкая при $x\neq y$.
\end{mydef}
...
\begin{myproof}
Для фундаментального решения имеем формулу Грина из теоремы \ref{th:green-formula}:
\[
    u(y)=
    ...
    \right)\dd S(x)
\]
Напишем вторую формулу Грина для функции $v=\widetilde{g}$ и функции $u$:
\[
...
\[
    ...
    \int\limits_{\p\Omega}\phi(x)\dfrac{\p g(x,y)}{\p\nu_x}\,\dd S(x).
\]
Необходимость формулы представления доказана. Обратно, если $u$ задаётся этой формулой, то непосредственная подстановка показывает, что она удовлетворяет уравнению и граничному условию задачи Дирихле. Это проверяется непосредственной подстановкой в уравнение и граничное условие.
\end{myproof}

\begin{myremark}
    Непосредственно подставлять $v(x)=g(x,y)$ во вторую формулу Грина нельзя, поскольку при фиксированном $y$ функция $g(x,y)$ имеет особенность в точке $x=y$ и не принадлежит $C^2(\Omega)$ на всей области. Поэтому формулу Грина сначала применяют в области $\Omega\setminus\overline{B_\varepsilon(y)}$, где функция Грина гладкая, а затем переходят к пределу $\varepsilon\to0$.
\end{myremark}
\begin{mylemma}
...
\[
    ...
    v(x)=g(x,\eta)
\]
Так как обе функции обращаются в нуль на $\p\Omega$, то формула Грина для этих функций принимает вид:
\[
    \int\limits_{\Omega}
    \left(
...
Выберем точки так, чтобы выполнялось:
\[|x|\cdot|x^*|=R^2\]
Точки $x$ и $x^*$ лежат на одном луче, выходящем из центра шара. Точку $x^*$ называют инверсией точки $x$ относительно шара $B_R(0).$
...
\begin{mylemma}
    Для точки, принадлежащей границе шара, справедливо равенство:
    \[
|x-y| = \dfrac{|x|}{R}|x^* - y|
\]
\end{mylemma}
...
\begin{myproof}
    Доказательство основано на подобии двух треугольников $\Delta Oxy$ и $Oyx^*$:
...
\begin{myexe}
    Показать, что для всех $x,y\in B_R(0)$ выполняется:
...
\begin{myproof}
    Приведём доказательство теоремы.\\
...
\begin{mydef}
    Пусть $g(x,y)$ --- это функция Грина в $\Omega$ для задачи Дирихле. Ядром Пуассона называется функция:
\[
...
Теперь в явном виде получим ядро Пуассона для шара. Будем рассматривать случай $n\geq 3$, но на самом деле для $n=2$, ответ не меняется.\\
Запишем функцию Грина и аккуратно возьмём от неё градиент:
...
Для производной по направлению имеем:
\[\nu_y=\dfrac{y}{R}\]
...
\end{mylemma}
\begin{myproof}
    Свойства 1, 2 и 4 следуют непосредственно из определения.\\
    3: $\Delta_xK(x,y)=\Delta_x\dfrac{\p g(x,y)}{\p \nu_y}=0$\\
    Свойство 5 получается из свойства 4 применением формулы к постоянной гармонической функции $u\equiv1$.
\end{myproof}


\begin{mytheorem}
    Пусть $\phi \in \C(\p B_R),$ тогда решение задачи Дирихле через ядро Пуассона обладает следующими свойствами:
...
\begin{myproof}
    \begin{enumerate}
        \item Дифференцируя, все становится ясно:
        \[D^{\alpha}u(x)=\int\limits_{\p B_R(0)}D^{\alpha}K(x,y)\phi(y)\dd S(y)\]
        \item Применяя оператор Лапласа под знаком интеграла, получаем: \[\Delta u(x) = \int\limits_{\p B_R(0)}\Delta_xK(x,y)\phi(y)\dd S(y)=0\]
        \item Зафиксируем точку $y_0\in \p B_R(0)$ и запишем:
...
\]
Предположим, что $|x-y_0|<\dfrac{\delta}{2}$, тогда $|x-y|\leq\dfrac{\delta}{2}$, и получаем оценку для ядра Пуассона:
...
Поскольку последнее слагаемое стремится к нулю при $x\to y_0$, для достаточно близких к $y_0$ точек $x$ получаем $|u(x)-\phi(y_0)|<2\varepsilon$. В силу произвольности $\varepsilon>0$ отсюда следует $u(x)\to\phi(y_0)$.
\end{enumerate}
\end{myproof}


\subsection{Внешняя задача Дирихле}
Вспомним нашу задачу и некоторые соотношения:
\begin{figure}[h!]
\centering
\begin{tikzpicture}[scale=1.4]
...
Учитывая ориентацию нормали во внешней задаче, получаем ядро Пуассона:
\[
...
\begin{mytheorem}
     Пусть $\phi \in \C(\p B_R(0)),$ тогда решение задачи Дирихле вне шара,  через ядро Пуассона обладает следующими свойствами:
...
\begin{myproof}
\phantomsection\label{proof:exterior-harmonic-decay}
    Ранее была сформулирована лемма~\ref{lem:exterior-harmonic-decay}; докажем её здесь для доказательства текущей теоремы:
    ...
    Существует $M: |x|>M$, поэтому функцию можно оценить следующим образом:
...
Уравнение Лежандра на $x\in[-1,1]$ при $n\in\N\cup\{0\}$ имеет вид:
\[\dfrac{\dd }{\dd x}\skobka{(1-x^2)\dfrac{\dd y(x)}{\dd x}}+n(n+1)y(x)=0\]
В итоговом виде уравнение выглядит так:
...
\begin{myproof}
Выпишем соотношения, необходимые для доказательства:
\[\skobka{(x^2-1)^n}'=2nx(x^2-1)^{n-1}\]
\[(x^2-1)\skobka{(x^2-1)^n}'=2nx(x^2-1)^{n}\]
К последнему выражению применим производную:
...
Вычислим аналогичную производную от $2nx(x^2-1)^{n}$:
...
Полученное решение позволяет ввести полиномы Лежандра следующим образом:
\begin{mydef}
...
\begin{enumerate}
    ...
    Для определённости рассмотрим на случай $m>n$. Воспользуемся уравнением Лежандра для $P_n$ и $P_m$:
\[
\int\limits_{-1}^{1}
...
Остаётся рассмотреть случай $m=n$. В этом случае:
...
\dd x =\boxed{=}
\]
Остаётся вычислить следующий интеграл, который мы обозначим через \(I_n\):
\[
I_n
=
...
\]
\item $\operatorname{Span}\{P_k(x)\}_{k=0}^{n} $ = \{множество полиномов степени $\leq n$\} = $\operatorname{Span}\{x^k\}_{k=0}^{n}$
\end{enumerate}
...
\begin{myremark}
    ...
    \[-\skobka{(1-x^2)y'}'=\lambda y,\] где $p(x)=1-x^2$, $q(x)=0$, $r(x)=1$. Заметим что $p(x)>0$ при $x\in[-1,1]$. Когда не существует собственных функций одно из двух линейно независимых решений ограничено на одном конце отрезка и расходится на другом, а второе обладает противоположным поведением. Когда же собственные функции есть, то одно решение расходится на обоих краях, а как полиномы Лежандра -- это собственные функции задачи Штурма-Лиувилля, а $\lambda_n=n(n+1) $, которые ограничены на обоих краях. Поэтому полиномы Лежандра -- это единственные решения уравнения Лежандра, которые ограничены на отрезке $[-1;1]$.\\
...
\begin{mylemma}
    Следующие функции называются присоединёнными функциями Лежандра:
    \[
P_{n}^m(x) = (1-x^2)^{m/2}\dfrac{\dd^m}{\dd x^m}P_n(x)б
\]
Они являются решениями следующего уравнения:
\[
...
\begin{myproof}
    Для доказательства достаточно выполнить следующее упражнение:\\
    \begin{myexe}
...
\begin{myremark}
    ...
    \item Присоединённые функции Лежандра также ортогональны в $\L_2(-1,1) $, откуда сразу следует из 5 пункта, что других решений нет.
\end{enumerate}
\end{myremark}
...
Сначала выведем формулу для оператора Лапласа в полярных координатах на $\R^2$. 
\[\Delta = \dfrac{\p^2}{\p x^2_1} + \dfrac{\p^2}{\p x^2_2}=f\left(\dfrac{\p}{\p \varphi}, \dfrac{\p}{\p r}\right)\]
Положим
\[ 
r=\sqrt{x_1^2+x_2^2},\qquad
x_1=r\cos\varphi,\qquad
x_2=r\sin\varphi
\]
Тогда
\[
\frac{\partial r}{\partial x_1}=\cos\varphi,
\qquad
\frac{\partial \varphi}{\partial x_1}=-\frac{\sin\varphi}{r},
\]
и поэтому
\[
...
\]
После вычисления вторых производных и сложения получаем
\[
\boxed{
\Delta f
=
...
+
\frac1{r^2}\frac{\partial^2 f}{\partial\varphi^2}
}.
\]

\begin{myremark}
В цилиндрических координатах $(r,\varphi,z)$ к этой формуле добавляется производная по $z$:
\[
\Delta
=
...
Теперь перейдём к сферическим координатам в $\R^3$:
\[
...
Если ввести
\[
w=\sqrt{x_1^2+x_2^2}=r\sin\theta,
\]
то переход от цилиндрических координат к сферическим даёт
\[
\boxed{
\Delta
=
...
То что было проделано выше является арифметическим упражнением с большим количеством выкладок, которые необходимо проделать самим. 
\begin{mydef}
Оператор
\[
\Delta_{S^2}
...
\frac{\partial^2}{\partial\varphi^2}
\]
называется \textbf{оператором Лапласа--Бельтрами} на единичной сфере $S^2$.
\end{mydef}

Таким образом лаплас в $\R^3$ равен
\[
\boxed{
\Delta
=
...
\begin{myexe}
Для перехода к сферическим координатам самостоятельно вычислить:
\begin{enumerate}
    \item матрицу Якоби
    \[
    J=\frac{D(r,\theta,\varphi)}{D(x_1,x_2,x_3)};
    \]
    \item $\det J$;
    \item операторы $\partial/\partial x_1$, $\partial/\partial x_2$, $\partial/\partial x_3$ через производные по $r,\theta,\varphi$;
    \item оператор Лапласа $\Delta$
\end{enumerate}
\end{myexe}

\begin{myremark}
    Стоит заметить, что первые два слагаемых в лапласе в $\R^3$ можно переписать так:
    \[\frac{\partial^2}{\partial r^2}
+
\frac2r\frac{\partial}{\partial r} =\dfrac{1}{r^2}\dfrac{\p}{\p r }r^2\dfrac{\p}{\p r }\]
\end{myremark}

\begin{myremark}
В $\R^n$ аналогичная формула имеет вид
\[
\boxed{
\Delta
=
...
+
\frac1{r^2}\Delta_{S^{n-1}}
}
\]
\end{myremark}

\begin{myexe}
Пусть $u,v\in C^2(S^2)$. Тогда cтоит показать, что
\[
\int_{S^2}(\Delta_{S^2}u)\,\overline v\,\dd S
=-
\int_{S^2}\left\langle \nabla_{S^2}u,\nabla_{S^2}v\right\rangle\dd S,
\]
а следовательно,
\[
\int_{S^2}(\Delta_{S^2}u)\,\overline v\,\dd S
=
\int_{S^2}u\,\overline{\Delta_{S^2}v}\,\dd S
\]
То есть оператор $\Delta_{S^2}$ симметричен в $L_2(S^2)$.
\end{myexe}
...
\begin{mydef}
Функция $Y\in C^\infty(S^2)$ называется \textbf{сферической функцией}, если она является собственной функцией оператора $-\Delta_{S^2}$:
\[
-\Delta_{S^2}Y=\lambda Y
\]
\end{mydef}

Ищем решение методом разделения переменных:
\[
Y(\theta,\varphi)=A(\theta)B(\varphi)
\]
Подставляя это представление в уравнение
\[
-\Delta_{S^2}Y=\lambda Y,
\]
получаем
\[
-\frac{B''(\varphi)}{B(\varphi)}
-
\frac{\sin\theta}{A(\theta)}
\frac{\dd}{\dd\theta}
\left(
\sin\theta A'(\theta)
\right)
...
\lambda\sin^2\theta
\]
Первая часть зависит только от $\varphi$, поэтому вводим константу разделения $\mu$:
\[
-\frac{B''}{B}=\mu
\]
Из периодичности по $\varphi$ следует
\[
B(\varphi)=e^{im\varphi},
\quad
m\in\Z,
\quad
\mu=m^2
...
\qquad
\frac{\dd}{\dd z}
=-\frac1{\sin\theta}\frac{\dd}{\dd\theta}
\]
Тогда уравнение переходит в обобщённое уравнение Лежандра:
\[
\frac{\dd}{\dd z}
\left(
(1-z^2)\frac{\dd A}{\dd z}
\right)
+
\lambda A
-
...
=0
\]
Ограниченные решения существуют при
\[
\lambda=n(n+1),
\qquad
n\ge |m|
...
\begin{mydef}
Сферические функции степени $n$ и порядка $m$ называют следующую вещь:
\[
Y_n^m(\theta,\varphi)
=
P_n^{|m|}(\cos\theta)e^{im\varphi},
\qquad
n=|m|,|m|+1,\ldots, \quad m \in \Z
\]
При этом выполняется следующие уравнение
\[
-\Delta_{S^2}Y_n^m=n(n+1)Y_n^m
\]
\end{mydef}
...
Функция
\[
r^nY_n^m(\theta,\varphi) =Q
\]
является однородным гармоническим полиномом порядка $n$ $(\operatorname{deg} Q=n)$ от переменных $x_1,x_2,x_3$.
\end{mylemma}
...
Значит, $r^nY_n^m$ является однородным полиномом порядка $n$ от
$x_1,x_2,x_3$.

Далее напишим лаплас в сферических координатах:
\[
\Delta\left(r^nY_n^m\right)
=
Y_n^m
\frac{1}{r^2}
\frac{\partial}{\partial r}
\left(
r^2\frac{\partial r^n}{\partial r}
\right)
+
\frac{r^n}{r^2}\Delta_{S^2}Y_n^m.
\]
...
\begin{mylemma}
Пусть $Q(x_1,x_2,x_3)$ --- однородный гармонический полином степени $n$:
\[
\Delta Q=0
\]
Тогда
\[
Q(x)=r^nY(\theta,\varphi),
\]
где $Y$ является сферической функцией степени $n$:
\[
-\Delta_{S^2}Y=n(n+1)Y
\]
\end{mylemma}

\begin{myproof}
Из однородности $Q$ имеем
\[
Q(x)=r^nQ\left(\frac{x}{r}\right)=r^nY(\theta,\varphi)
\]
Подставляя в $\Delta Q=0$, получаем
\[
n(n+1)r^{n-2}Y+r^{n-2}\Delta_{S^2}Y=0,
\]
то есть
\[
-\Delta_{S^2}Y=n(n+1)Y
...
\begin{myremark}
    Другими словами, сферическая функция является сужением однородного гармонического полинома на сферу
\end{myremark}

\begin{myexample}
Пространства однородных гармонических полиномов имеют следующие первые базисы:
\begin{enumerate}
    \item $n=0$: константы, $\dim=1$;
    \item $n=1$: $x_1,x_2,x_3$, $\dim=3$;
    \item $n=2$: например,
    \[
    x_1^2-x_3^2,\quad
    x_2^2-x_3^2,\quad
    ...
    поэтому $\dim=5$.
\end{enumerate}
Вообще размерность пространства сферических функций степени $n$ равна $2n+1$.
\end{myexample}

\begin{mylemma}
Сферические функции ортогональны. То есть в нашем случае:
\[
\int_{S^2}Y_n^m(\theta,\varphi)
\overline{Y_{\widetilde n}^{\widetilde m}(\theta,\varphi)}\,\dd S
=
\frac{4\pi}{2n+1}
\frac{(n+|m|)!}{(n-|m|)!}
\delta_{n\widetilde n}\delta_{m\widetilde m}
\]
\end{mylemma}
...
\begin{mytheorem}
Оператор Лапласа--Бельтрами $-\Delta_{S^2}$ имеет собственные значения
\[
\lambda_n=n(n+1),
\qquad
n\in\mathbb N\cup\{0\},
...
Базис в собственном подпространстве, соответствующем $\lambda_n$, можно выбрать в виде
\[
Y_n^m(\theta,\varphi)
=
P_n^{|m|}(\cos\theta)e^{im\varphi},
...
Эта система полна и ортогональна в $L_2(S^2)$.
\end{mytheorem}

Замыкание линейной оболочки системы $\{Y_n^m\}$ совпадает со всем пространством
$L_2(S^2)$. Это утверждение эквивалентно полноте системы.
...
В $L_2(S^2)$ плотны непрерывные функции, а непрерывные функции можно приблизить
гладкими. Поэтому существует
\[
\widetilde f\in C^\infty(S^2):
\qquad
\|\widetilde f-f\|_{L_2(S^2)}<\varepsilon
\]

Зафиксируем $\theta$. Тогда функцию $\widetilde f(\theta,\varphi)$ можно разложить
в ряд Фурье по переменной $\varphi$:
\[
\widetilde f(\theta,\varphi)
=
\sum_{m=-\infty}^{+\infty}
\widetilde f_m(\theta)e^{im\varphi}
\]

Так как $\widetilde f\in C^\infty(S^2)$ и сфера компактна, функция
$\widetilde f$ равномерно непрерывна. Поэтому для любого $\varepsilon>0$
существует $\delta>0$ такое, что
\[
\rho\bigl((\theta_1,\varphi_1),(\theta_2,\varphi_2)\bigr)<\delta
\]
откуда сразу ясно
\[
\left|
\widetilde f(\theta_1,\varphi_1)
-
\widetilde f(\theta_2,\varphi_2)
\right|
<\varepsilon
\]

Разобьём сферу на конечное число поясов по переменной $\theta$ так, чтобы
ширина каждого пояса была меньше $\delta$.

Для конечного числа выбранных значений $\theta_j$ существует такое $N$, что
одновременно для всех $j$
\[
\left|
\widetilde f(\theta_j,\varphi)
-
\sum_{m=-N}^{N}
\widetilde f_m(\theta_j)e^{im\varphi}
\right|
<\varepsilon
\]
...
Теперь для каждого фиксированного $m$ функцию $\widetilde f_m(\theta)$
можно приблизить конечной линейной комбинацией присоединённых функций
Лежандра:
\[
\left\|
\widetilde f_m(\theta)
-
\sum_n c_{n,m}P_n^{|m|}(\cos\theta)
\right\|_{L_2(0,\pi)}
<
\frac{\varepsilon}{N}
\]
...
\right\|_{L_2(S^2)}
<
\frac{2\pi\varepsilon}{N}
\]

Таким образом, конечная сумма
\[
\sum_{m=-N}^{N}
\sum_n
c_{n,m}
P_n^{|m|}(\cos\theta)e^{im\varphi}
\]
приближает $\widetilde f$, а следовательно, и $f$, в $L_2(S^2)$.

Но каждое слагаемое
\[
P_n^{|m|}(\cos\theta)e^{im\varphi}
\]
является сферической функцией. Следовательно, система сферических функций
полна в $L_2(S^2)$.
...
=
\left\{
\text{однородные полиномы степени }k
\text{ в }\mathbb R^n
\right\},
\]
\[
...
Элементы $\mathcal H_k$ будем рассматривать после ограничения на сферу
$S^{n-1}$ как подпространства в $L_2(S^{n-1})$.
...
\[
\int_{B_1(0)}
\langle\nabla P,\nabla\overline Q\rangle\,dx
=
-
\int_{B_1(0)}
\Delta P\,\overline Q\,dx
+
\int_{S^{n-1}}
...
Поскольку $P$ является однородным полиномом степени $k$, на единичной сфере
\[
\frac{\partial P}{\partial\nu}=kP
\]
...
Аналогично, меняя $P$ и $Q$ местами, получаем
\[
\int_{B_1(0)}
\langle\nabla P,\nabla\overline Q\rangle\,dx
=
...
Посмотрим на размерности пространств.

Для однородных полиномов степени $k$ от $n$ размерность пространтсва, схожа с задачей о распредедении мячиков в корзины.
\[
\dim\mathcal P_k
=
\binom{k+n-1}{n-1}
...
Действительно, необходимо распределить степень $k$ между переменными
\[
x_1,\ldots,x_n,
\]
то есть посчитать число одночленов
\[
...
В $\mathbb R^3$
\[
\dim\mathcal H_k=2k+1
\]
\end{myremark}


Так как пространства $\mathcal H_k$ попарно ортогональны,
\[
\bigoplus_{k=0}^{\infty}\mathcal H_k
\subset
L_2(S^{n-1})
\]
...
Последовательно применяя разложение
\[
\mathcal P_k
=
\mathcal H_k
...
Q_1\in\mathcal H_k,
\qquad
Q_2\in\mathcal H_{k-2},
\qquad
Q_3\in\mathcal H_{k-4},
\quad\ldots
\]
...
Поэтому сужение любого полинома на сферу является конечной суммой
элементов пространств $\mathcal H_j$.

Сужение полиномов на сферу плотны в $L_2(S^{n-1})$
...
Теперь найдём собственные значения оператора Лапласа--Бельтрами.
...
Так как $Q$ однороден степени $k$, то
\[
\frac{\partial Q}{\partial r}
=
\frac{k}{r}Q,
\qquad
\frac{\partial^2Q}{\partial r^2}
=
\frac{k(k-1)}{r^2}Q
\]
...
$-\Delta_{S^{n-1}}$, а собственными функциями являются ограничения
гармонических однородных полиномов на сферу.

\begin{myremark}
Итак, сферические функции в $n$-мерном случае можно рассматривать как
сужение гармонических однородных полиномов на единичную сферу.
\end{myremark}
...
При $k\geq1$ кратность собственного значения равна $2$, и соответствующее
собственное подпространство порождается функциями
\[
e^{ik\varphi},
\qquad
e^{-ik\varphi}
...
При $k=0$ кратность равна $1$, а собственной функцией является -- 
$
1$
\end{myexample}
...
\subsection{Производящая функция для полиномов Лежандра (в $\R^3$)}
...
Выберем точку $x_0$ на единичной сфере на положительной полуоси $x_3$:
\[
x_0:
\qquad
r=1,
...
x\neq x_0,
\]
можно применить оператор Лапласа к ряду почленно:
...
\sum_{n=0}^{\infty}
\left(
n(n+1)r^{n-2}C_n(\theta)
+
r^{n-2}\Delta_{S^2}C_n(\theta)
\right) \Rightarrow
-\Delta_{S^2}C_n(\theta)
=
n(n+1)C_n(\theta)
...
Так как $C_n$ не зависит от $\varphi$, получаем уравнение Лежандра, поэтому
\[
C_n(\theta)
=
\alpha_nP_n(\cos\theta).
...
или, обозначая $x=\cos\theta$, получим

\[
\frac1{\sqrt{1-2rx+r^2}}
=
\sum_{n=0}^{\infty}
P_n(x)r^n,
\qquad
...
\begin{mytheorem}
Для $|r|<1$ и $|\mu|\le1$ справедливо разложение
\[
\frac1{\sqrt{1-2\mu r+r^2}}
=
\sum_{n=0}^{\infty}P_n(\mu)r^n
\]
Правая часть называется производящей функцией полиномов Лежандра.
\end{mytheorem}

\begin{myproof}
При фиксированном $\mu$ функция
\[
F(z)=\frac1{\sqrt{1-2\mu z+z^2}}
\]
аналитична в достаточно малом круге с центром в нуле. 
...
\frac{1}{2\pi i}
\oint_{\gamma}
\frac{1}{\sqrt{1-2\mu z+z^2}}
\frac{\dd z}{z^{n+1}}
\]

Сделаем замену переменной так:
\[
\sqrt{1-2\mu z+z^2}=1-\xi z
\]

Тогда
\[
1-2\mu z+z^2=(1-\xi z)^2
=
1-2\xi z+\xi^2z^2 
\Rightarrow
z^2(1-\xi^2)-2z(\mu-\xi)=0\Rightarrow
z=\frac{2(\mu-\xi)}{1-\xi^2}
\]
...
\dd z
=
-\frac{2(1-2\mu\xi+\xi^2)}
{(1-\xi^2)^2}\,\dd\xi
\]



Подставляя это в интеграл для $P_k(\mu)$, получаем
\[
P_n(\mu)
=
-\frac{1}{2\pi i}
...
\begin{myremark}
Из доказательства получается также интегральное представление Шлефли:
\[
P_n(\mu)
=
\frac1{2^n 2\pi i}
\oint_\gamma
\frac{(\zeta^2-1)^n}{(\zeta-\mu)^{n+1}}\,\dd\zeta,
\]
где контур $\gamma$ охватывает точку $\mu$ и не пересекает ветви при $\pm1$.
\end{myremark}

\begin{myexe}
Проверить следующие свойства функции $P_\nu$:
\begin{enumerate}
    \item
    $
    \left((1-x^2)P_\nu'(x)\right)'
    ...
    \item $P_\nu(1)=1$;
    \item при $x\to-1$ $P_\nu(x) \sim \sin(\pi\nu)\ln(1+x)$;
    \item Если взять контур $\gamma=\{\xi:|\xi-\mu|=\sqrt{1-\mu^2}\}$, то справедливо интегральное представление
\[
P_\nu(\mu)
=
\frac1{2\pi}
\int_0^{2\pi}
\left(
\mu+i\sqrt{1-\mu^2}\sin\varphi
\right)^\nu\dd\varphi,
\qquad -1<\mu<1
\]
    \item для целого $n\ge0$ верно
    $
    |P_n(x)|\le1,
    \qquad x\in[-1,1]
    $
...
\begin{mylemma}
\textbf{Формула Лапласа.}\par\smallskip

Пусть $Y_\ell$ --- сферическая функция степени $\ell$, для которой верно:
\[
-\Delta_{S^2}Y_\ell=\ell(\ell+1)Y_\ell,
\qquad
Y_\ell=\sum_{m=-\ell}^{\ell}a_mY_\ell^m
\]
Обозначим точки единичной сферы через $\omega,\widetilde\omega\in S^2$. Тогда для $n,\ell\ge0$ выполняется формула Лапласа
\[
\int_{S^2}
P_n\left(\left\langle\omega,\widetilde\omega\right\rangle\right)
Y_\ell(\widetilde\omega)\,\dd S(\widetilde\omega)
=
\frac{4\pi}{2\ell+1}
\delta_{n\ell}
Y_\ell(\omega)
...
\begin{myproof}
Рассмотрим гармоническую функцию $u$, для которой верна формула Грина:
\[u(x)=\int_{S^2}\left(\frac{\partial u}{\partial\nu}(y)\frac{1}{4\pi|x-y|}-u(y)\frac{\partial}{\partial\nu}\frac{1}{4\pi|x-y|}\,\right)\dd S(y),\quad x\in B_1(0)\]
\[
u(x)=r^\ell Y_\ell(\omega),
\qquad x=(r,\omega),\quad r<1
\]
Для $y=(\widetilde r,\widetilde\omega)$, $\widetilde r>r$, производящая функция даёт
\[
\frac1{|x-y|}
= \dfrac{1}{\sqrt{r^2+\widetilde r^2-2r\widetilde r\left\langle\omega,\widetilde\omega\right\rangle}}=
\sum_{n=0}^{\infty}
P_n\left(\left\langle\omega,\widetilde\omega\right\rangle\right)
\frac{r^n}{\widetilde r^{n+1}}
\]
При $\widetilde r=1$
\[
\frac{\partial}{\partial\nu}\frac{1}{|x-y|}\Big|_{\widetilde r=1}
=-
\sum_{n=0}^{\infty}(n+1)
P_n\left(\left\langle\omega,\widetilde\omega\right\rangle\right)r^n
\]
Подставляя эти разложения в формулу Грина для гармонической функции $u$, получаем
\[
r^\ell Y_\ell(\omega)
=
\frac1{4\pi}
...
Y_\ell(\widetilde\omega)\,\dd S(\widetilde\omega)
\]
Сравнивая коэффициенты при одинаковых степенях $r$, получаем равенство двух степенных рядов $\Rightarrow$ равенство почленно.
\end{myproof}

\begin{myremark}
Если
$
f(\omega)=\sum_{n=0}^{\infty}Y_n(\omega)
$
--- разложение функции по пространствам сферических гармоник степени $n$, то
\[
Y_n(\omega)
=
\frac{2n+1}{4\pi}
...
f(\widetilde\omega)
P_n\left(\left\langle\omega,\widetilde\omega\right\rangle\right)
\dd S(\widetilde\omega)
\]
\end{myremark}

\subsection{Краевые задачи для уравнения Лапласа в шаре и вне шара}

\textbf{Внутренняя задача Дирихле.}\par\smallskip
Пусть для задачи
\[
\begin{cases}
\Delta u=0, & r<R,\\
u|_{r=R}=\psi(\theta,\varphi)
\end{cases}
\]
граничную функцию разложим по сферическим гармоникам:
\[
\psi(\theta,\varphi)
=
\sum_{n=0}^{\infty}Y_n(\theta,\varphi),
\qquad
-\Delta_{S^2}Y_n(\theta,\varphi)
=
n(n+1)Y_n(\theta,\varphi)
\]
...
Используя формулу Лапласа для сферических функций,
\[
Y_n(\omega)
=
\frac{2n+1}{4\pi}
\int_{S^2}
P_n\!\left(\langle\omega,\widetilde\omega\rangle\right)
Y_n(\widetilde\omega)\,\dd S(\widetilde\omega),
\]
получаем
\[
u(x)
...
поэтому ряд можно записать через производную фундаментального решения:
\[
u(x)
=
\frac{1}{4\pi}
...
R\frac{1}{|x-y|}
\right)
\psi(y)\,\dd S(y)=\int_{|y|=R}
\frac{R^2-|x|^2}{4\pi R|x-y|^3}
\psi(y)\,\dd S(y)\]
...
Необходимо условие 
\[
\int_{|y|=R}\psi(y)\,\dd S(y)=0
\]

Снова разложим граничное условие по сферическим функциям:
\[
\psi(\theta,\varphi)
=
\sum_{n=1}^{\infty}Y_n(\theta,\varphi)
\]
...
\frac{2}{r}u_n'(r)
-
\frac{n(n+1)}{r^2}u_n(r)
=
0
\]

Сделаем замену Эйлера
\[
u_n(r)=r^\alpha
\]
Тогда
...
\alpha=-n-1
\]
Получаем два решения, но для внешней задачи выбираем убывающее, то есть:
\[
u_n(r)=C_nr^{-n-1}
\]
...
\begin{myexe}
Показать, что решение внешней задачи Дирихле можно записать в интегральном виде
\[
u(x)
=
\int_{|y|=R}
...
Она аналитична по $z$ в $B_1(0)\subset\C$ и раскладывается в следующий ряд
\[
\frac{1}{1-z}
e^{-\frac{xz}{1-z}}
=
\sum_{n=0}^{\infty}L_n(x)z^n
\]
\end{mydef}
...
Следовательно, подставляя в выражение выше, получим,
\[
L_n(x)
=
\frac{e^x}{2\pi i}
...
Вспомним  формулу Коши для производной:
\[
\frac{1}{2\pi i}
\oint_{\Gamma_x}
\frac{f(\xi)}
...
\]
\begin{mydef}
    Полиномом Лагерра называют следующую структуру:
    \[L_n(x)=
\frac{e^x}{n!}
\frac{\dd^n}{\dd x^n}
\left(x^ne^{-x}\right)\]
\end{mydef}
Первые несколько полиномов Лагерра выглядят так:
\[L_0(x)=1;\quad L_1(x)=1-x;\quad L_2(x)=\dfrac{x^2-4x+2}{2}\]

\subsection{Свойства полиномов Лагерра}

\begin{enumerate}
    \item
    \[
    \deg L_n=n,\quad L_n(x)
    =
    \frac{(-1)^n}{n!}x^n+\ldots
    \]
    ...
    \frac{1}{n!}L_m(x)
    \frac{\dd^n}{\dd x^n}
    \left(x^ne^{-x}\right)\dd x
    =
    \frac{(-1)^n}{n!}
    \int_0^\infty
    \frac{\dd^n}{\dd x^n}L_m(x)\,
    x^ne^{-x}\,\dd x
    \]
    ...
    =
    \begin{cases}
    1,\,\text{стоит честно посчитать и получить единицу},  & n=m,\\
    0, & n>m.
    \end{cases}
    \]

    \item Полином $L_n$ является решением уравнения
    \[
    xL_n''(x)+(1-x)L_n'(x)+nL_n(x)=0
    \]

    Для вывода воспользуемся формулой Родрига. Положим
    \[
    f=x^ne^{-x}\Rightarrow
    f'=(nx^{n-1}-x^n)e^{-x}\Rightarrow
    xf'
    =
    (nx^n-x^{n+1})e^{-x}
    =
    ...
    Продифференцируем это равенство $n+1$ раз:
    \[
    xf^{(n+2)}+(n+1)f^{(n+1)}
    =
    nf^{(n+1)}
    ...
    Подстановим в наше уравнение и получим то, что от нас требовалось
    \[
    x(L_n''-2L_n'+L_n)
    +(1+x)(L_n'-L_n)
    +(n+1)L_n=0\Rightarrow
    xL_n''+(1-x)L_n'+nL_n=0
    \]

    \item
    \begin{myexe}
    Проверить рекуррентное соотношение
    \[
    (n+1)L_{n+1}(x)
    -
    (2n+1-x)L_n(x)
...
Уравнение Лагерра можно записать как
\[
xL_n''+(1-x)L_n'+nL_n=0,
\]
или
...
В форме задачи Штурма--Лиувилля на $[0,+\infty)$:
\[
p(x)=xe^{-x},
\qquad
q(x)=0,
...
Собственные числа:
\[
\lambda_n=0,1,\ldots,n,\ldots
\]
и собственные функции:
\[
L_0,L_1,\ldots,L_n,\ldots
\]
\end{myremark}
\subsection{Обобщённые полиномы Лагерра}
...
\begin{mydef}
Обобщённые полиномы Лагерра определяются формулой
\[
L_n^s(x)
=
\frac{1}{n!}
x^{-s}e^x
\frac{\dd^n}{\dd x^n}
\left(x^{n+s}e^{-x}\right)
\]
\end{mydef}
\begin{myexe}
    Проверить, что их производящая функция такова:
\[
\Psi_s(z,x)
=
\frac{
...
\begin{enumerate}
    \item $L_n^s$ --- полином степени $n$:
    \[ \operatorname{deg}L_n^s = n;\quad
    L_n^s(x)
    =
    \frac{(-1)^n}{n!}x^n+\ldots
    \]
    ...
    \[
    \int_0^\infty
    L_n^s(x)L_m^s(x)x^se^{-x}\,\dd x
    =\dfrac{1}{n!}
    \int_0^\infty
    \left(x^{n+s}e^{-x}\right)^{(n)}
    L_m^s(x)\,\dd x=\dfrac{(-1)^{n}}{n!}\int_0^\infty
    \left(x^{n+s}e^{-x}\right)^{(n)}
    \dfrac{\dd L_m^s(x)}{\dd x^n}\dd x =0
    \]
        \item
\begin{myexe}
    \[
    ...
    Эквивалентно задачи Ш-Л  на $[0,+\infty)$,
    \[
    (x^{s+1}e^{-x}y')'
    +
    \lambda x^se^{-x}y
...
\begin{mylemma}
    Пусть $s>-1$ и $\psi_n^s(x)=x^{s/2}e^{-x/2}L_n^s(x)$
Тогда соответсвующая задача Ш-Л
\[
(x\psi')'
+
\left(
\lambda-\frac{x}{4}-\frac{s^2}{4x}
\right)\psi
=
0,
\qquad
...
\subsection{Квантовая механика за час}
...
Плотностью вероятности обнаружить частицу в $\Omega$ принято называть следующую вещь:
\[
\int_{\Omega}|\psi(t,x)|^2\,\dd x
\]
значит по всему пространству, получаем условие нормировки
\[
\int_{\R^3}|\psi(t,x)|^2\,\dd x=1
\]
...
Выберем такую систему единиц, в которой:
\[
\hbar=m=e=1,
\qquad
Z=1
...
\frac2rX'(r)Y
+
\frac1{r^2}X(r)\Delta_{S^2}Y
\right)
-
\frac{XY}{r}
=
EXY\Big| \cdot -\frac{2r^2}{XY}
\]
...
Левая часть зависит только от $r$, правая --- только от углов, поэтому обе равны константе:
\[
-\Delta_{S^2}Y
=
\ell(\ell+1)Y
...
Сведём его к уравнению из предыдущего пункта. Положим
\[
X(r)
=
\frac{1}{\sqrt r}\varphi(r)
\]
...
X'
=
\frac{1}{\sqrt r}\varphi'
-
\frac{1}{2r^{3/2}}\varphi,
\]
\[
X''
=
...
Сравнивая с уравнением для обобщённых полиномов Лагерра, получаем
\[
\lambda=\frac{1}{\sqrt{-2E}},
\qquad
s=2\ell+1
\]
...
Волновая функция выглядит следующим образом:
\[
...
Перепишим уровни энергии, как мы позже получим на квант.мехе(со всеми $const$):
\[
E_n
=
-\frac{mZ^2e^4}{2\hbar^2n^2}
\]
...
Число состояний при фиксированном $n$:
\[
\sum_{\ell=0}^{n-1}(2\ell+1)=n^2
\]

$n$ --- главное квантовое число, $\ell$ --- азимутальное, $m$ --- магнитное квантовое число.
...
\Gamma(z)
=
\int_0^1t^{z-1}e^{-t}\,\dd t
+
\int_1^\infty t^{z-1}e^{-t}\,\dd t
\]
...
\begin{enumerate}
    ...
    Далее равенство продолжается аналитически на допустимые $z$.
    ...
    Чтобы доказать это свойство, используем
    \[
    \Gamma(z)
    =
    \lim_{n\to\infty}
    ...
    \frac{1}{n}
    \int_0^n
    e^{-t}t^{\operatorname{Re}z+1}\,\dd t
    \longrightarrow0
    \]

    С другой стороны, при замене
    \[
    t=n\tau
    \]
    получаем
    ...
    =
    n^z
    \frac{\Gamma(n+1)\Gamma(z)}
    {\Gamma(n+z+1)}
    \]

    \item Формула удвоения:
 \begin{mylemma}
    \[ \sqrt\pi\,\Gamma(2z)
    =
    2^{2z-1}
...
\begin{myproof}
    ...
    Используем предельное представление:
    \[
    \Gamma(2z)
    =
    \lim_{n\to\infty}
    ...
    и аналогично рассматриваем
    \[
    \Gamma(z)\Gamma\left(z+\frac12\right)
    \]

    После разделения чётных и нечётных множителей получаем
    \[
    \frac{
    2^{2z-1}
...
\item Бесконечное произведение для $1/\Gamma(z)$
...
\lim_{n\to\infty}
\left(
1+\frac12+\frac13+\ldots+\frac1n-\ln n
\right)
\approx0.5772
\]
\end{mylemma}
...
a_n
=
1+\frac12+\ldots+\frac{1}{n-1}-\ln n,
\qquad
n\ge2
\]
...
a_2=1-\ln2,
\]
и рекуренкое соотношение
\[
a_n-a_{n+1}
=
-\ln n+\ln(n+1)-\frac1n
=
\ln\left(1+\frac1n\right)-\frac1n<0
\]
...
Кроме того,
\[
\frac12+\frac13+\ldots+\frac1{n-1}
<
\int_1^{n-1}\frac{\dd x}{x}
=
\ln(n-1)
\]
...
a_n<1+\ln(n-1)-\ln n<1,
\]
то есть последовательность ограничена и предел существует.

Из предельного представления гамма-функции:
\[
\frac1{\Gamma(z)}
=
\lim_{n\to\infty}
...
Для сходимости используем
\[
\ln\left(
\left(1+\frac zk\right)e^{-z/k}
\right)
=
\ln\left(1+\frac zk\right)-\frac zk
=
O\left(\frac1{k^2}\right).
\]

Поэтому произведение сходится.

\begin{myremark}
Функция
$
\frac1{\Gamma(z)}$
является целой и ее нули в $z=0,-1,2 \ldots \Rightarrow \Gamma(z) \neq 0$
\end{myremark}
...
\begin{mydef}
Уравнением Бесселя порядка $\nu$ называется уравнение
\[
w''(z)
+
\frac1z w'(z)
...
z^2w''+zw'+(k^2z^2-\nu^2)w=0
\]
сводится к уравнению Бесселя заменой
\[
\xi=kz,
\qquad
\widetilde w(\xi)=w(z)
...
=
z^\rho
\sum_{m=0}^{\infty}c_mz^m
=
\sum_{m=0}^{\infty}c_mz^{m+\rho}
\]
...
Далее продолжая все тоже самое
\[
c_{2k}
=
\frac{
...
\begin{mydef}
Функцией Бесселя первого рода порядка $\nu$ называется
\[
J_\nu(z)
=
\sum_{k=0}^{\infty}
...
\subsection{Свойства функций Бесселя}

    \[
    J_\nu(z)
    =
    z^\nu\cdot\text{целая функция от }z^2
    \]
\begin{mylemma}
    
    При фиксированном $z$ функция $J_\nu(z)$ зависит от $\nu$.
    ...
    J_{-\nu}(z)
    \]
    являются решениями уравнения Бесселя.
...
$
  ...
  \begin{mylemma}
    ...
    \]
        ...
        Очевидно, что бессель при $n\to-n$ симмметричен.
    Действительно,
    \[
    J_{-n}(z)
    =
    ...
    \frac{(-1)^k}
    {k!\Gamma(k-n+1)}
    \left(\frac z2\right)^{2k-n}
    \]

    Первые $n$ слагаемых равны нулю, поскольку
    \[
    \frac1{\Gamma(k-n+1)}=0,
    \qquad
    k=0,\ldots,n-1
    ...
    \frac{(-1)^{m+n}}
    {(m+n)!\,m!}
    \left(\frac z2\right)^{2m+n}
    =
    (-1)^nJ_n(z)
    \]
        \end{myproof}
    \begin{myremark}
    При $n\in\Z$ второе линейно независимое решение содержит $\ln z$.
    \end{myremark}
 ...
 \begin{mylemma}
    ...
    J_{1/2}(z)
    =
    \sqrt{\frac{2}{\pi z}}\sin z,
  \quad
    J_{-1/2}(z)
    =
    \sqrt{\frac{2}{\pi z}}\cos z
    \]
 \end{mylemma}
 \begin{myproof}
    Для первой формулы:
    ...
    \]
    получаем разложение синуса.

    Аналогично для $J_{-1/2}$ получается разложение косинуса.
 \end{myproof}
\begin{mylemma}
    \[
    \left(z^\nu J_\nu(z)\right)'
...
\end{mylemma}
\begin{myproof}
    Первая формула следует из ряда:
    \[
    z^\nu J_\nu(z)
    =
    \sum_{k=0}^{\infty}
...
\subsection{Производящая функция для $J_n(z)$}

\begin{mytheorem}
При
$
t\in\C\setminus\{0\}\setminus\{\infty \}
$
справедливо следующие разложение в ряд Лорана
\[
e^{\frac z2\left(t-\frac1t\right)}
=
\sum_{n\in\Z}
J_n(z)t^n
\]
...
Заменим, что при определенной замене ничего не меняется:
\[
t\mapsto-\frac1t \Rightarrow a_n=(-1)^na_{-n},
\]
поэтому формула верна для всех $n\in\Z$.
\end{myproof}
...
\frac1\pi
\int_0^\pi
\cos(z\sin\varphi-n\varphi)\,\dd\varphi
\]
\end{mytheorem}

\begin{myproof}
В производящей функции положим
\[
t=e^{i\varphi},
\qquad
\varphi\in\R
...
\int_{-\pi}^{\pi}
e^{iz\sin\varphi}
e^{-in\varphi}\,\dd\varphi
\]

Беря вещественную часть:
\[
J_n(z)
=
\frac1{2\pi}
...
z\ge0
$
бессель имеет положительные нули
\[
j_{\nu,1},
j_{\nu,2},
j_{\nu,3},
...
\subsection{Спектр оператора Лапласа в круге и в шаре}

В $\R^2$ рассмотрим $u\in \C^2$, для которой выполнена задача
\[
\begin{cases}
-\Delta u=\lambda u, & |x|<R,\\
u|_{|x|=R}=0
\end{cases}
\]
Так как задача в круге и шаре, то будем использовать полярные координаты, то есть $x=(r,\varphi)$
...
Положим
\[
f_m(r)=w(\sqrt\lambda\,r)
\]

Получаем уравнение Бесселя. Из ограниченности в окрестности $r=0$:
\[
f_m(r)=J_m(\sqrt\lambda\,r).
\]

Из условия на границе, получим
\[
f_m(R)=0
\Rightarrow
\sqrt\lambda R=j_{m,k} \Rightarrow
\lambda_{m,k}
=
\left(\frac{j_{m,k}}R\right)^2
\]
...
J_m\left(
\frac{j_{m,k}}Rr
\right)e^{\pm im\varphi},
\qquad
m\ge0,\quad k\in\N
\]

Кратность собственных чисел:
\[\lambda_{m,k}=
\begin{cases}
1, & m=0,\\
2, & m\in\N
...
Теперь рассмотрим задачу в $\R^3$:
\[
\begin{cases}
-\Delta u(x)=\lambda u(x), & |x|<R,\\
u|_{|x|=R}=0
\end{cases}
\]
...
=
\frac1{\sqrt r}
w(\sqrt\lambda\,r)\end{vmatrix}=
w''
+
\frac1\rho w'
+
\left(
1-\frac{(m+\frac12)^2}{\rho^2}
\right)w
=
0.
\]

Из-за ограниченности $g$ в $r=0$:
\[
w=J_{m+1/2}.
\]

Граничное условие дает нам собственные числа:
\[
g(R)=0
\quad\Rightarrow\quad
\sqrt\lambda R
=
j_{m+1/2,k}\Rightarrow
\lambda_{m,k}
=
\left(
\frac{j_{m+1/2,k}}R
...
Cоответсвующие собственные функции:
\[
u_{m,k}(x)
=
\frac1{\sqrt r}
...
При
\[
m=0,\qquad k=1,\qquad Y_0=\operatorname{const}
\Rightarrow
J_{1/2}(z)
=
...
\section{Функции Ханкеля}

\subsection{Метод Лапласа в теории дифференциальных уравнений}
...
ze^{tz}
=
\frac{\partial}{\partial t}e^{tz},
\]
интегрируем первую часть по частям:
\[
-\int_C
\left(
(t^2+p_0t+q_0)v(t)
...
+
\left.
(t^2+p_0t+q_0)v(t)e^{tz}
\right|_{\partial C}
=
0
\]

Тогда обнуляя внеинтегральный член, получаем
\[
\left(
(t^2+p_0t+q_0)v
\right)'
...
=
\frac{\rho_1-1}{t-\alpha_1}
+
\frac{\rho_2-1}{t-\alpha_2}\,\dd t,\qquad \alpha_1\neq\alpha_2
\]
...
При этом контур должен удовлетворять
\[
\left.
(t-\alpha_1)^{\rho_1}
(t-\alpha_2)^{\rho_2}
...
\begin{myremark}
    \begin{enumerate}
        \item В случае, когда вещественные части обоих переменных положительны$
\operatorname{Re}\rho_1>0,\,
\operatorname{Re}\rho_2>0,
$ контур выглядит так:
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.3\linewidth]{pics/12.png}
        \caption{}
    \end{figure}

\item Если же выполенено следующие условие $\rho_1-\rho_2\in\Z$, то контур такой:    \begin{figure}[H]
        \centering
        \includegraphics[width=0.3\linewidth]{pics/13.png}
        \caption{}
    \end{figure}
\item В общем случае 
\[w_{1,2}z=\int_{C_{1,2}}
(t-\alpha_1)^{\rho_1-1}
(t-\alpha_2)^{\rho_2-1}
e^{tz}\,\dd t\] контур выглядит так:
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.3\linewidth]{pics/14.png}
        \caption{}
    \end{figure}
    \end{enumerate}
\end{myremark}
\begin{mylemma}
Контурное представление обратной гамма-функции выглядит следующим образом:
\[
\frac1{\Gamma(z)}
=
\frac1{2\pi i}
...
При обходе отрицательной вещественной полуоси аргумент отличается на $2\pi$, поэтому
\[
\int_{\widetilde C}
e^{-t}t^{z-1}\,\dd t
=
...
\frac{\pi}{\sin\pi z},
\]
получаем контурное представление для $1/\Gamma(z)$.
\end{myproof}

Рассмотрим асимптотику интегрального решения при
\[
z\to+\infty
\]

Для решения, связанного с точкой $\alpha_1$:
\[
w_1(z)
=
\int_{C_1}
...
Основной вклад даёт окрестность $\alpha_1$. Поэтому
\[
w_1(z)
\sim
(\alpha_1-\alpha_2)^{\rho_2-1}
\int
(t-\alpha_1)^{\rho_1-1}
e^{tz}\,\dd t =\begin{vmatrix}
    t-\alpha_1=\tau
\end{vmatrix}
\sim
(\alpha_1-\alpha_2)^{\rho_2-1}
e^{\alpha_1z}
\int
\tau^{\rho_1-1}e^{\tau z}\,\dd\tau=\begin{vmatrix}
s=\tau z\end{vmatrix}
\sim\]\[\sim
\frac{
2\pi i
...
\operatorname{Re}z>0
\]
Если же $\rho_1\in\N,$ то $w_1(z)=0$
\end{mylemma}

\subsection{Функции Ханкеля}
...
Поэтому решение уравнения Бесселя выглядит так:
\[
u_{1,2}(z)
=
\int_{C_{1,2}}
...
\begin{mydef}
Функции Ханкеля $H_\nu^{(1)}(z)$ и $H_\nu^{(2)}(z)$ --- решения уравнения Бесселя, которые имеют асимптотики
\[
H_\nu^{(1)}(z)
\sim
\sqrt{\frac2{\pi z}}
e^{\,i\left(z-\frac{\pi}{4}(2\nu+1)\right)},
\quad
z\to+\infty
\]
\[
...
На самоосознание остается сделать один шаг, чтобы получить интегральное представление функций Ханкеля:
...
% =================================================
% СВЯЗЬ ФУНКЦИЙ ХАНКЕЛЯ И БЕССЕЛЯ
% =================================================

\subsection{Связь функций Ханкеля и Бесселя}

Напомним, что после замены
$
w(z)=z^\nu u(z)
$,
уравнение Бесселя принимает вид
\[
zu''(z)+(2\nu+1)u'(z)+zu(z)=0
\]
...
\rho_1-\rho_2\in\Z,
$
поэтому решение можно представить интегралом
\[
u(z)
=
\int_C
...
Контур $C$ обходит точки ветвления $t=i$ и $t=-i$.
...
=
\int_C
(t^2+1)^{\nu-\frac12}\,\dd t=\begin{vmatrix}
t=i\tau\end{vmatrix}
\boxed{=}\]

При переходе с одного берега разреза на другой аргумент множителя
$(t^2+1)^{\nu-\frac12}$ изменяется на $2\pi$, поэтому
\[
\boxed{=}
i
\left(
...
\nu\notin\frac12+\Z
$
Тогда функция Бесселя допускает интегральное представление
\[
J_\nu(z)
=
\frac{
...
e^{tz}\,\dd t
\]
Это следует из ассимптотики бесселя в нуле.
\end{mylemma}
...
Отдельно необходимо рассматривать полуцелые значения
\[
\nu=
\frac12,\frac32,\frac52,\ldots
\quad \text{и} \quad
...
Разобьём контур $C$ на две части, соответствующие контурам,
используемым в интегральных представлениях функций Ханкеля.
...
По формуле отражения для гамма-функции
\[
\Gamma\left(\nu+\frac12\right)
\Gamma\left(\frac12-\nu\right)
=
...
Для функций Ханкеля при $z\to+\infty$ справделивы следующие асимптотики:
\[
H_\nu^{(1)}(z)
=
\sqrt{\frac{2}{\pi z}}
...
\subsection{Функции Неймана}

\begin{mydef}

Функцией Неймана порядка $\nu$ называется функция
\[
N_\nu(z)
=
\frac{
...
\]
Функция $N_\nu(z)$ называется также функцией Бесселя второго рода.
\end{mydef}
Все функции, которые решаюют уравнение Бесселя, являются циклическими.\\

Функции Неймана являются линейно независимыми с $J_\nu$
решениями уравнения Бесселя.

Используя асимптотики функций Ханкеля, получаем
\[
N_\nu(z)
=
\sqrt{\frac{2}{\pi z}}
...
Тогда подставяляя и пользуясь тригонометрическими тождествами, получаем
\[
\cos(A+\pi\nu)
=
\cos A\cos\pi\nu
...
Отсюда получаем нужную нам формулу:
\[
N_\nu(z)
=
\frac{
...
Из полученных формул следуют следующие соотношения.
...
=
\sqrt{\frac{2}{\pi z}}
(\sin z-i\cos z)= -i\sqrt{\frac{2}{\pi z}}e^{iz}
\]
...
Пусть
$
n\in\Z$, тогда в окрестности $z=0$ справедлива формула
\[
N_n(z)
=
\frac{2}{\pi}
...
При $\nu\to n$ числитель и знаменатель стремятся к нулю, поскольку
\[
J_{-n}(z)=(-1)^nJ_n(z)
\]
...
Далее, дифференцируя степенной ряд для $J_\nu(z)$ по параметру $\nu$,
выделяем логарифмический член
\[
\frac{2}{\pi}
\ln\frac z2\,J_n(z),
\]
а оставшаяся часть имеет вид
\[
z^{-|n|}\omega_n(z),
\qquad
\omega_n\in\mathrm{Hol}(\C)
