...
\\
1) Для любых $a<b$ существует функция $\eta \in\DR$ (гладкая с
компактным носителем) такая, что
\[
\eta(x) = 1 \quad \text{при } x\in[a,b].
\]

Берём стандартную бамп-функцию $\psi\in \DR$,
\[
\psi(x)=
\begin{cases}
\exp\!\bigl(-\tfrac{1}{1-x^2}\bigr), & |x|<1,\\[1mm]
0, & |x|\ge 1,
\end{cases}
\]
и нормируем её так, чтобы получить гладкую функцию, равную $1$ на
$[a,b]$ и равную $0$ вне немного большего отрезка. Полученная функция
лежит в $\DR$ и удовлетворяет $\eta|_{[a,b]}=1$.
...
\begin{myproof}
Доказательство будет в 11 параграфе(ссылку оставить потом)
\end{myproof}
...
Сюда надо картинку и пояснение как мы строим все точки
