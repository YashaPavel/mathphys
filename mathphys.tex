...
\newtcolorbox{myexample}[1][]{
  enhanced,
  breakable,
  colback=examplebackback,
  colframe=exampleframeframe,
  boxrule=0.8pt,
  arc=2mm,
  fonttitle=\bfseries,
...
$$(f\circ C, \phi)=\int\limnits_Rf(C(x))\phi(x)\dd x=\int\limits_Rf(y)\phi(C^{-1}(y))\dfrac{\dd y}{|C'(C^{-1}y)|}$$ \\
Первое равенство обусловлено тем, что $f\in L_{1, loc}$, т.е. \ref{zero}.
\begin{mydef}
    Пусть $f\in \DDR$, $\quad C$ удовлетворяет \ref{first}. 
    $$(f\circ C, \phi)=\int\limnits_Rf(C(x))\phi(x)\dd x:=\bigg(f, \dfrac{\phi(C^{-1}(\cdot))}{|C'(C^{-1}(\cdot))|})\bigg)$$
\end{mydef} \\
